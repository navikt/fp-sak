...
class FinnAktivitetsavtalerForUtbetalingsgrad {
    ...
    static List<AktivitetsAvtale> finnAktivitetsavtalerSomSkalBrukes(Collection<AktivitetsAvtale> avtalerAAreg,
                                                                     LocalDate jordmorsdato,
                                                                     LocalDate termindato) {
        var avtalerSomOverlapperMedPeriode = avtalerAAreg.stream()
        ...
        if (avtalerSomOverlapperMedPeriode.isEmpty()) {
            avtalerSomOverlapperMedPeriode = avtalerAAreg.stream()
                .filter(a -> a.getPeriode().overlapper(DatoIntervallEntitet.fraOgMedTilOgMed(jordmorsdato, termindato)))
                .toList();
            if (avtalerSomOverlapperMedPeriode.isEmpty()) {
    ...
    private static List<AktivitetsAvtale> finnAvtaleVedStart(Collection<AktivitetsAvtale> avtalerAAreg,
                                                             LocalDate jordmorsdato,
