...
public class YtelserKonsolidertTjeneste {
     ...
     */
    public List<TilgrensendeYtelser> utledYtelserRelatertTilBehandling(AktørId aktørId,
                                                                       InntektArbeidYtelseGrunnlag grunnlag,
                                                                       Set<RelatertYtelseType> inkluder) {
        ...
        var resultatÅpen = fagsakRepository.hentForBruker(aktørId)
            .stream()
            .filter(sak -> !saksnumre.contains(sak.getSaksnummer()))
            .filter(sak -> inkluder.isEmpty() || inkluder.contains(
                BehandlingRelaterteYtelserMapper.mapFraFagsakYtelseTypeTilRelatertYtelseType(sak.getYtelseType())))
     ...
     * behandling i VL (som ennå ikke har vedtak).
     */
    public List<TilgrensendeYtelser> utledAnnenPartsYtelserRelatertTilBehandling(AktørId aktørId,
                                                                                 InntektArbeidYtelseGrunnlag grunnlag,
                                                                                 Set<RelatertYtelseType> inkluder) {
    ...
    private TilgrensendeYtelser mapFraFagsakForAnnenPart(Fagsak fagsak, LocalDate periodeDato) {
        ...
        return behandlingRepository.finnSisteAvsluttedeIkkeHenlagteBehandling(fagsak.getId())
            .map(b -> justerPeriode(b, tilgrensendeYtelser, periodeDato))
    ...
    private TilgrensendeYtelser setFamilieHendelseDatoForES(Fagsak fagsak, TilgrensendeYtelser tilgrensendeYtelser) {
        try {
            return behandlingRepository.hentSisteYtelsesBehandlingForFagsakIdReadOnly(fagsak.getId())
                ...
                .map(d -> new TilgrensendeYtelser(tilgrensendeYtelser.relatertYtelseType(), d, d, tilgrensendeYtelser.statusNavn(),
                    tilgrensendeYtelser.saksNummer()))
                .orElse(tilgrensendeYtelser);
        } catch (Exception e) {
    ...
    private boolean erSisteBehandlingAvsluttetAvslag(Fagsak fagsak) {
        ...
        return behandling.filter(Behandling::erSaksbehandlingAvsluttet).isPresent() && behandling.flatMap(
                b -> behandlingsresultatRepository.hentHvisEksisterer(b.getId()))
