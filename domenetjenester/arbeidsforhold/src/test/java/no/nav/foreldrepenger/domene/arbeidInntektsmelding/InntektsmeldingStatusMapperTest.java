...
class InntektsmeldingStatusMapperTest {
    ...
    void skal_teste_alle_im_mottatt() {
        ...
        var arbeidsforholdInntektsmeldinger = InntektsmeldingStatusMapper.mapInntektsmeldingStatus(Map.of(ag, Set.of(ref)), Map.of(),
            Collections.emptyList());

        assertThat(arbeidsforholdInntektsmeldinger).isNotNull().hasSize(1);
        assertThat(arbeidsforholdInntektsmeldinger.getFirst().inntektsmeldingStatus()).isEqualTo(
    ...
    void skal_teste_im_mangler() {
        ...
        var arbeidsforholdInntektsmeldinger = InntektsmeldingStatusMapper.mapInntektsmeldingStatus(Map.of(ag, Set.of(ref)), Map.of(ag, Set.of(ref)),
            Collections.emptyList());
    ...
    void skal_teste_en_im_mangler_en_er_mottatt() {
        ...
        var arbeidsforholdInntektsmeldinger = InntektsmeldingStatusMapper.mapInntektsmeldingStatus(Map.of(ag, Set.of(ref1, ref2)),
            Map.of(ag, Set.of(ref2)), Collections.emptyList());

        assertThat(arbeidsforholdInntektsmeldinger).isNotNull().hasSize(2);
        var mottattIM = arbeidsforholdInntektsmeldinger.stream().filter(ai -> ai.ref().gjelderFor(ref1)).findFirst().orElseThrow();
        assertThat(mottattIM.inntektsmeldingStatus()).isEqualTo(ArbeidsforholdInntektsmeldingStatus.InntektsmeldingStatus.MOTTATT);
        var ikkeMottattIM = arbeidsforholdInntektsmeldinger.stream().filter(ai -> ai.ref().gjelderFor(ref2)).findFirst().orElseThrow();
