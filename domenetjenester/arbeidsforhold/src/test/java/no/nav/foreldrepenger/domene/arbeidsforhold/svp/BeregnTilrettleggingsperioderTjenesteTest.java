...
class BeregnTilrettleggingsperioderTjenesteTest {
    ...
    void skal_beregne_perioder() {
        ...
        var svpGrunnlag = new SvpGrunnlagEntitet.Builder().medBehandlingId(behandling.getId())
            .medOpprinneligeTilrettelegginger(
                List.of(opprettTilrettelegging(jordmorsdato, delvisTilrettelegging, BigDecimal.valueOf(40), InternArbeidsforholdRef.nullRef())))
            .build();
    ...
    void skal_beregne_perioder_riktig_ved_to_arbeidsforhold_i_samme_bedrift() {
        ...
        var arb1 = tilretteleggingMedUtbelingsgrad.stream()
            .filter(t -> t.getTilretteleggingArbeidsforhold().getInternArbeidsforholdRef().gjelderFor(ARB_1))
            .findFirst()
            .get();
        var arb2 = tilretteleggingMedUtbelingsgrad.stream()
            .filter(t -> t.getTilretteleggingArbeidsforhold().getInternArbeidsforholdRef().gjelderFor(ARB_2))
            .findFirst()
            .get();
    ...
    void skal_beregne_perioder_riktig_ved_to_arbeidsforhold_i_samme_bedrift_der_det_ene_er_inaktivt_og_man_søker_uten_arbeidsforholdId() {
        ...
        var svpGrunnlag = new SvpGrunnlagEntitet.Builder().medBehandlingId(behandling.getId())
            .medOpprinneligeTilrettelegginger(
                List.of(opprettTilrettelegging(jordmorsdato, delvisTilrettelegging, BigDecimal.valueOf(50), InternArbeidsforholdRef.nullRef())))
            .build();
    ...
    private SvpTilretteleggingEntitet opprettTilrettelegging(LocalDate jordmorsdato,
                                                             LocalDate delvis,
        ...
        return new SvpTilretteleggingEntitet.Builder().medKopiertFraTidligereBehandling(false)
            ...
            .medTilretteleggingFom(new TilretteleggingFOM.Builder().medFomDato(delvis)
    ...
    private FamilieHendelseEntitet byggFamilieHendelse(Behandling behandling, LocalDate termindato) {
        var søknadHendelse = familieHendelseRepository.opprettBuilderFor(behandling.getId()).medAntallBarn(1);
        søknadHendelse.medTerminbekreftelse(
            søknadHendelse.getTerminbekreftelseBuilder().medTermindato(termindato).medUtstedtDato(termindato.minusDays(21)));
    ...
    private void lagAktørArbeid(InntektArbeidYtelseAggregatBuilder inntektArbeidYtelseAggregatBuilder,
        ...
        var yrkesaktivitetBuilder = aktørArbeidBuilder.getYrkesaktivitetBuilderForNøkkelAvType(opptjeningsnøkkel, ArbeidType.ORDINÆRT_ARBEIDSFORHOLD);
        var aktivitetsAvtaleBuilder = yrkesaktivitetBuilder.getAktivitetsAvtaleBuilder(DatoIntervallEntitet.fraOgMedTilOgMed(fom, tom), true);
    ...
    private void lagAktørArbeidMed2ArbeidsforholdIsammeVirksomhet(InntektArbeidYtelseAggregatBuilder inntektArbeidYtelseAggregatBuilder,
        ...
        var aktivitetsAvtaleBuilder1 = yrkesaktivitetBuilder1.getAktivitetsAvtaleBuilder(DatoIntervallEntitet.fraOgMedTilOgMed(førsteFom, førsteTom),
            true);

        var aktivitetsAvtale1 = aktivitetsAvtaleBuilder1.medPeriode(DatoIntervallEntitet.fraOgMedTilOgMed(førsteFom, førsteTom));

        var prosent1 = yrkesaktivitetBuilder1.getAktivitetsAvtaleBuilder(DatoIntervallEntitet.fraOgMedTilOgMed(førsteFom, Tid.TIDENES_ENDE), false);
        prosent1.medProsentsats(BigDecimal.valueOf(75));

        yrkesaktivitetBuilder1.leggTilAktivitetsAvtale(aktivitetsAvtale1)
        ...
        var yrkesaktivitetBuilder2 = aktørArbeidBuilder.getYrkesaktivitetBuilderForNøkkelAvType(opptjeningsnøkkel,
            ArbeidType.ORDINÆRT_ARBEIDSFORHOLD);
        var aktivitetsAvtaleBuilder2 = yrkesaktivitetBuilder2.getAktivitetsAvtaleBuilder(DatoIntervallEntitet.fraOgMedTilOgMed(andreFom, andreTom),
            true);

        var aktivitetsAvtale2 = aktivitetsAvtaleBuilder2.medPeriode(DatoIntervallEntitet.fraOgMedTilOgMed(andreFom, andreTom));

        var prosent2 = yrkesaktivitetBuilder2.getAktivitetsAvtaleBuilder(DatoIntervallEntitet.fraOgMedTilOgMed(andreFom, Tid.TIDENES_ENDE), false);
        prosent2.medProsentsats(BigDecimal.valueOf(80));

        yrkesaktivitetBuilder2.leggTilAktivitetsAvtale(aktivitetsAvtale2)
