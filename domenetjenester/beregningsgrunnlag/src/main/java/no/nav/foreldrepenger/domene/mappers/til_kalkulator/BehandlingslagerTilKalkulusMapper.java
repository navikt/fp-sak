...
public class BehandlingslagerTilKalkulusMapper {
    ...
    public static BeregningsgrunnlagDto mapBeregningsgrunnlag(BeregningsgrunnlagEntitet beregningsgrunnlagFraFpsak) {
        ...
        //med
        builder.medGrunnbeløp(Beløp.fra(Optional.ofNullable(beregningsgrunnlagFraFpsak.getGrunnbeløp()).map(v -> v.getVerdi()).orElse(null)));
        ...
        if (beregningsgrunnlagFraFpsak.getSammenligningsgrunnlag().isPresent() && beregningsgrunnlagFraFpsak.getSammenligningsgrunnlagPrStatusListe()
            .isEmpty()) {
            // Intill alle grunnlag er migrert til nytt sammenligningsgrunnlag må denne være her
            BGMapperTilKalkulus.mapGammeltTilNyttSammenligningsgrunnlag(beregningsgrunnlagFraFpsak).forEach(builder::leggTilSammenligningsgrunnlag);
        }

        //lister
        beregningsgrunnlagFraFpsak.getAktivitetStatuser()
            .forEach(beregningsgrunnlagAktivitetStatus -> builder.leggTilAktivitetStatus(
                BGMapperTilKalkulus.mapAktivitetStatus(beregningsgrunnlagAktivitetStatus)));
        beregningsgrunnlagFraFpsak.getBeregningsgrunnlagPerioder()
            .forEach(beregningsgrunnlagPeriode -> builder.leggTilBeregningsgrunnlagPeriode(
                BGMapperTilKalkulus.mapBeregningsgrunnlagPeriode(beregningsgrunnlagPeriode)));
        builder.leggTilFaktaOmBeregningTilfeller(
            beregningsgrunnlagFraFpsak.getFaktaOmBeregningTilfeller().stream().map(KodeverkTilKalkulusMapper::mapFaktaBeregningTilfelle).toList());
        beregningsgrunnlagFraFpsak.getSammenligningsgrunnlagPrStatusListe()
            .forEach(sammenligningsgrunnlagPrStatus -> builder.leggTilSammenligningsgrunnlag(
                BGMapperTilKalkulus.mapSammenligningsgrunnlagMedStatus(sammenligningsgrunnlagPrStatus)));
    ...
    public static BeregningRefusjonOverstyringerDto mapRefusjonOverstyring(BeregningRefusjonOverstyringerEntitet refusjonOverstyringerFraFpsak) {
        ...
        refusjonOverstyringerFraFpsak.getRefusjonOverstyringer().forEach(beregningRefusjonOverstyring -> {
            var refusjonsperioder = beregningRefusjonOverstyring.getRefusjonPerioder()
                .stream()
                .map(BehandlingslagerTilKalkulusMapper::mapRefusjonperiode)
                .toList();
            var dto = new BeregningRefusjonOverstyringDto(IAYMapperTilKalkulus.mapArbeidsgiver(beregningRefusjonOverstyring.getArbeidsgiver()),
                beregningRefusjonOverstyring.getFørsteMuligeRefusjonFom().orElse(null), refusjonsperioder,
    ...
    public static BeregningRefusjonPeriodeDto mapRefusjonperiode(BeregningRefusjonPeriodeEntitet refusjonPeriodeEntitet) {
        return new BeregningRefusjonPeriodeDto(
            refusjonPeriodeEntitet.getArbeidsforholdRef() == null ? null : IAYMapperTilKalkulus.mapArbeidsforholdRef(
                refusjonPeriodeEntitet.getArbeidsforholdRef()), refusjonPeriodeEntitet.getStartdatoRefusjon());
    }
    ...
    private static Consumer<BeregningAktivitetEntitet> mapAktivitet(BeregningAktivitetAggregatDto.Builder dtoBuilder) {
        return beregningAktivitet -> {
            ...
            builder.medArbeidsforholdRef(beregningAktivitet.getArbeidsforholdRef() == null ? null : IAYMapperTilKalkulus.mapArbeidsforholdRef(
                beregningAktivitet.getArbeidsforholdRef()));
            builder.medArbeidsgiver(
                beregningAktivitet.getArbeidsgiver() == null ? null : IAYMapperTilKalkulus.mapArbeidsgiver(beregningAktivitet.getArbeidsgiver()));
    ...
    private static Intervall mapDatoIntervall(ÅpenDatoIntervallEntitet periode) {
        return periode.getTomDato() == null ? Intervall.fraOgMed(periode.getFomDato()) : Intervall.fraOgMedTilOgMed(periode.getFomDato(),
            periode.getTomDato());
    }
    ...
    public static BeregningAktivitetOverstyringerDto mapAktivitetOverstyring(BeregningAktivitetOverstyringerEntitet beregningAktivitetOverstyringerFraFpsak) {
        ...
        beregningAktivitetOverstyringerFraFpsak.getOverstyringer().forEach(overstyring -> {
            ...
            builder.medArbeidsforholdRef(
                overstyring.getArbeidsforholdRef() == null ? null : IAYMapperTilKalkulus.mapArbeidsforholdRef(overstyring.getArbeidsforholdRef()));
            ...
            builder.medHandling(
                overstyring.getHandling() == null ? null : KodeverkTilKalkulusMapper.mapBeregningAktivitetHandling(overstyring.getHandling()));
    ...
    public static BeregningsgrunnlagGrunnlagDto mapGrunnlag(BeregningsgrunnlagGrunnlagEntitet beregningsgrunnlagFraFpsak) {
        ...
        beregningsgrunnlagFraFpsak.getBeregningsgrunnlag()
            .ifPresent(beregningsgrunnlagDto -> oppdatere.medBeregningsgrunnlag(mapBeregningsgrunnlag(beregningsgrunnlagDto)));
        beregningsgrunnlagFraFpsak.getOverstyring()
            .ifPresent(beregningAktivitetOverstyringerDto -> oppdatere.medOverstyring(mapAktivitetOverstyring(beregningAktivitetOverstyringerDto)));
        oppdatere.medRegisterAktiviteter(mapRegisterAktiviteter(beregningsgrunnlagFraFpsak.getRegisterAktiviteter()));
        beregningsgrunnlagFraFpsak.getSaksbehandletAktiviteter()
            .ifPresent(
                beregningAktivitetAggregatDto -> oppdatere.medSaksbehandletAktiviteter(mapSaksbehandletAktivitet(beregningAktivitetAggregatDto)));
        beregningsgrunnlagFraFpsak.getRefusjonOverstyringer()
            .ifPresent(
                beregningRefusjonOverstyringerDto -> oppdatere.medRefusjonOverstyring(mapRefusjonOverstyring(beregningRefusjonOverstyringerDto)));
        beregningsgrunnlagFraFpsak.getBeregningsgrunnlag()
            .flatMap(BehandlingslagerTilKalkulusMapper::mapFaktaAggregat)
            .ifPresent(oppdatere::medFaktaAggregat);
    ...
    private static Optional<FaktaAktørDto> mapFaktaAktør(List<BeregningsgrunnlagPrStatusOgAndel> andeler,
    ...
    private static void mapEtterlønnSluttpakke(List<no.nav.foreldrepenger.domene.modell.kodeverk.FaktaOmBeregningTilfelle> faktaOmBeregningTilfeller,
        ...
        var harEtterlønnSlutpakke = faktaOmBeregningTilfeller.stream()
            .anyMatch(
                tilfelle -> tilfelle.equals(no.nav.foreldrepenger.domene.modell.kodeverk.FaktaOmBeregningTilfelle.FASTSETT_ETTERLØNN_SLUTTPAKKE));
