...
class MapOpptjeningTilKalkulusInputTest {
    ...
    void skal_ta_med_arbeidsforhold_når_ingen_inntektsmelding() {
        ...
        var resultat = MapOpptjeningTilKalkulusInput.mapOpptjening(new OpptjeningAktiviteter(p1), byggIAY(), REF, STPT);
    ...
    void skal_ta_med_flere_arbeidsforhold_når_inntektsmelding_uten_id() {
        ...
        var resultat = MapOpptjeningTilKalkulusInput.mapOpptjening(new OpptjeningAktiviteter(p1, p2, p3), byggIAY(), REF, STPT);
        assertThat(resultat.getPerioder()).hasSize(3);
        assertFinnes(resultat, orgnr, ref1);
    ...
    void skal_håndtere_aktiviteter_med_og_uten_id_i_hos_arbeidsgivere() {
        ...
        var resultat = MapOpptjeningTilKalkulusInput.mapOpptjening(new OpptjeningAktiviteter(p1, p2, p3, p4), byggIAY(), REF, STPT);
        assertThat(resultat.getPerioder()).hasSize(4);
    ...
    void flere_arbfor_samme_arbeidsgiver_noen_med_im_andre_uten() {
        ...
        var resultat = MapOpptjeningTilKalkulusInput.mapOpptjening(new OpptjeningAktiviteter(p1, p2, p3), byggIAY(), REF, STPT);
        assertThat(resultat.getPerioder()).hasSize(2);
        assertFinnes(resultat, orgnr1, ref1);
        assertFinnes(resultat, orgnr1, ref2);
    }
    ...
    private void assertFinnes(OpptjeningAktiviteterDto resultat, String orgnr, InternArbeidsforholdRef ref1) {
        var match = resultat.getPerioder()
            .stream()
            .filter(op -> op.getArbeidsgiver().getIdent().equals(orgnr) && op.getAbakusReferanse().getAbakusReferanse()
