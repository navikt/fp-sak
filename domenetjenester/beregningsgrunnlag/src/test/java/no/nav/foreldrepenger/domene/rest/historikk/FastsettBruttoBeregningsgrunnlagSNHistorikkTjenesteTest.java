...
public class FastsettBruttoBeregningsgrunnlagSNHistorikkTjenesteTest {
    ...
    void setUp(EntityManager entityManager) {
        ...
        fastsettBruttoBeregningsgrunnlagSNHistorikkTjeneste = new FastsettBruttoBeregningsgrunnlagSNHistorikkTjeneste(
            lagMockHistory());
        vurderVarigEndringEllerNyoppstarteteSNHistorikkTjeneste = new VurderVarigEndringEllerNyoppstarteteSNHistorikkTjeneste(
            lagMockHistory());
    }
    ...
    public void skal_generere_historikkinnslag_ved_fastsettelse_av_brutto_beregningsgrunnlag_SN() {
        // Arrange
        var varigEndring = true;
        var behandling = buildOgLagreBeregningsgrunnlag();

        //Dto
        var vurderVarigEndringEllerNyoppstartetSNDto = new VurderVarigEndringEllerNyoppstartetSNDto("begrunnelse1",
            varigEndring);
        var fastsettBGDto = new FastsettBruttoBeregningsgrunnlagSNDto("begrunnelse2", BRUTTO_BG);

        // Act
        vurderVarigEndringEllerNyoppstarteteSNHistorikkTjeneste.lagHistorikkInnslag(
            new AksjonspunktOppdaterParameter(behandling, Optional.empty(), vurderVarigEndringEllerNyoppstartetSNDto),
            vurderVarigEndringEllerNyoppstartetSNDto);
        fastsettBruttoBeregningsgrunnlagSNHistorikkTjeneste.lagHistorikk(
            new AksjonspunktOppdaterParameter(behandling, Optional.empty(), fastsettBGDto), fastsettBGDto);

        var historikkinnslag = new Historikkinnslag();
        historikkinnslag.setType(HistorikkinnslagType.FAKTA_ENDRET);
        var historikkInnslag = tekstBuilder.medHendelse(HistorikkinnslagType.FAKTA_ENDRET).build(historikkinnslag);

        // Assert
        assertThat(historikkInnslag).hasSize(2);

        var del1 = historikkInnslag.get(0);
        assertThat(del1.getEndredeFelt()).hasSize(1);
        assertHistorikkinnslagFelt(del1, HistorikkEndretFeltType.ENDRING_NAERING, null,
            HistorikkEndretFeltVerdiType.VARIG_ENDRET_NAERING.getKode());
        assertThat(del1.getBegrunnelse()).as("begrunnelse")
            .hasValueSatisfying(begrunnelse -> assertThat(begrunnelse).isEqualTo("begrunnelse1"));
        assertThat(del1.getSkjermlenke()).as("skjermlenkeOpt")
            .hasValueSatisfying(skjermlenke -> assertThat(skjermlenke).as("skjermlenke1")
                .isEqualTo(SkjermlenkeType.BEREGNING_FORELDREPENGER.getKode()));

        var del2 = historikkInnslag.get(1);
        ...
        assertThat(del2.getBegrunnelse()).as("begrunnelse")
            .hasValueSatisfying(begrunnelse -> assertThat(begrunnelse).isEqualTo("begrunnelse2"));
        assertThat(del2.getSkjermlenke()).as("skjermlenke2").isNotPresent();
    }

    private void assertHistorikkinnslagFelt(HistorikkinnslagDel del,
