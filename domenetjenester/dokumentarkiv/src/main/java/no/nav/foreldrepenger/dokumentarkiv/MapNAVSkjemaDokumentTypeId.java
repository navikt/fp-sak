...
class MapNAVSkjemaDokumentTypeId {
    ...
    private static final Map<DokumentTypeId, Integer> DOKUMENT_TYPE_RANK = Map.ofEntries(Map.entry(DokumentTypeId.INNTEKTSMELDING, 1),
        ...
        Map.entry(DokumentTypeId.KLAGE_ETTERSENDELSE, 10), Map.entry(DokumentTypeId.TILBAKEBETALING_UTTALSELSE, 11),
    ...
    public static DokumentTypeId velgRangertHovedDokumentType(Collection<DokumentTypeId> alleTyper) {
        var typerMedBeskrivelse = alleTyper.stream().filter(t -> !t.erAnnenDokType() && !t.erEttersendelseType()).collect(Collectors.toSet());
        var minrank = typerMedBeskrivelse.stream().map(MapNAVSkjemaDokumentTypeId::dokumentTypeRank).min(Comparator.naturalOrder()).orElse(MAX_RANK);
        if (minrank < MAX_RANK) {
