...
public class RegelBeregnFeriepenger implements RuleService<BeregningsresultatFeriepengerRegelModell> {
    ...
    public Specification<BeregningsresultatFeriepengerRegelModell> getSpecification() {
        ...
        var beregnFeriepengerEttÅr = rs.beregningsRegel(BeregnFeriepengerEttÅr.ID, BeregnFeriepengerEttÅr.BESKRIVELSE, new BeregnFeriepengerEttÅr(),
            new BeregnetFeriepenger());

        // FP_BR 8.6 Beregn feriepenger (Flere kalenderår)
        var beregnFeriepengerFlereÅr = rs.beregningsRegel(BeregnFeriepengerFlereÅr.ID, BeregnFeriepengerFlereÅr.BESKRIVELSE,
            new BeregnFeriepengerFlereÅr(), new BeregnetFeriepenger());

        // FP_BR 8.5 Går feriepengeperioden over flere kalenderår?
        var sjekkOmFeriepengePeriodenGårOverFlereÅr = rs.beregningHvisRegel(new SjekkOmFeriepengePeriodenGårOverFlereÅr(), beregnFeriepengerFlereÅr,
            beregnFeriepengerEttÅr);

        //FP_BR 8.4 Har bruker uttak i feriepengeperiode?
        var sjekkOmUttakIFeriepengePeriode = rs.beregningHvisRegel(new SjekkBrukerHarOmUttakIFeriepengePeriode(),
            sjekkOmFeriepengePeriodenGårOverFlereÅr, new BeregnetFeriepenger());

        //FP_BR 8.3 Finn brukers feriepengeperiode
        var finnBrukersFeriepengePeriode = rs.beregningsRegel(FinnBrukersFeriepengePeriode.ID, FinnBrukersFeriepengePeriode.BESKRIVELSE,
            new FinnBrukersFeriepengePeriode(), sjekkOmUttakIFeriepengePeriode);

        // FP_BR 8.8 Har bruker flere dager ignen av feriekvoten?
        var sjekkOmBrukerHarDagerIgjenAvFeriekvote = rs.beregningHvisRegel(new SjekkOmBrukerHarFeriekvoteTilgjengelig(), finnBrukersFeriepengePeriode,
            new BeregnetFeriepenger());

        // FP_BR 8.2 Har bruker fått utbetalt foreldrepenger i den totale stønadsperioden?
        var sjekkOmBrukerHarFåttUtbetaltFP = rs.beregningHvisRegel(new SjekkOmBrukerHarFåttUtbetaltForeldrepenger(),
            sjekkOmBrukerHarDagerIgjenAvFeriekvote, new BeregnetFeriepenger());
