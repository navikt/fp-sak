...
public class FeriepengeReberegnTjeneste {
    ...
    public boolean harDiffUtenomPeriode(Long behandlingId) {
        ...
        var gjeldendeResultat = beregningsresultatRepository.hentUtbetBeregningsresultat(behandlingId);
    ...
    private boolean utledNyttResultat(Behandling behandling, BeregningsresultatEntitet gjeldendeResultat, boolean loggAvvik) {
        var feriepengetjeneste = FagsakYtelseTypeRef.Lookup.find(beregnFeriepengerTjenesteInstance, behandling.getFagsakYtelseType()).orElseThrow();
