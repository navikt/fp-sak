...
public class VurderBehovForÅHindreTilbaketrekk {
    ...
    private static boolean andelMåVurderes(BRNøkkelMedAndeler revurderingBRNøkkelMedAndeler, BeregningsresultatAndel originalBrukersAndel) {
        List<BeregningsresultatAndel> brukersAndelIRevurdering = revurderingBRNøkkelMedAndeler.getAlleBrukersAndelMedReferanse(originalBrukersAndel.getArbeidsforholdRef());
    ...
    private static int dagsats(List<BeregningsresultatAndel> andeler) {
        return andeler.stream().mapToInt(BeregningsresultatAndel::getDagsats).sum();
    }
}
