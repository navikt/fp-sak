...
public final class OmfordelRevurderingsandelerMedReferanser {
    ...
    private static int getReberegnetDagsats(BeregningsresultatAndel revurderingAndel,
        ...
        if (revurderingAndel.erBrukerMottaker()) {
            int revurderingArbeidsgiverDagsats = revurderingNøkkelMedAndeler.getArbeidsgiversAndelMedReferanse(
                revurderingAndel.getArbeidsforholdRef()).map(BeregningsresultatAndel::getDagsats).orElse(0);
            reberegnetDagsats = OmfordelDagsats.beregnDagsatsBruker(revurderingAndel.getDagsats(), revurderingArbeidsgiverDagsats,
            ...
            int revurderingBrukersDagsats = revurderingNøkkelMedAndeler.getBrukersAndelMedReferanse(revurderingAndel.getArbeidsforholdRef())
                .map(BeregningsresultatAndel::getDagsats)
                .orElse(0);
            reberegnetDagsats = OmfordelDagsats.beregnDagsatsArbeidsgiver(revurderingAndel.getDagsats(), revurderingBrukersDagsats,
                originalBrukersDagsats);
        }
