...
class MapBRAndelSammenligningTidslinje {
    ...
    private static LocalDateTimeline<List<BeregningsresultatAndel>> identifiserUtbetaltPeriode(LocalDate dagensDato) {
        ...
        return new LocalDateTimeline<>(Tid.TIDENES_BEGYNNELSE, alleredeUtbetaltTom, Collections.emptyList());
    }
    ...
    private static LocalDateSegment<BRAndelSammenligning> combine(LocalDateInterval interval,
        ...
        var forrigeAndeler = Optional.ofNullable(forrigeSegment).map(LocalDateSegment::getValue).orElse(Collections.emptyList());
        var bgAndeler = Optional.ofNullable(bgSegment).map(LocalDateSegment::getValue).orElse(Collections.emptyList());
