...
class StartpunktUtlederPersonopplysning implements StartpunktUtleder {
    ...
    private List<StartpunktType> hentAlleStartpunktForPersonopplysninger(BehandlingReferanse ref,
                                                                         PersonopplysningGrunnlagEntitet grunnlag1,
                                                                         PersonopplysningGrunnlagEntitet grunnlag2) {
        ...
        if (forelderDødEndret) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UTTAKSVILKÅR,
                "foreldres død", grunnlag1.getId(), grunnlag2.getId());
            startpunkter.add(StartpunktType.UTTAKSVILKÅR);
        }
        if (poDiff.erSivilstandEndretForBruker()) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UTTAKSVILKÅR, "sivilstand",
                grunnlag1.getId(), grunnlag2.getId());
            startpunkter.add(StartpunktType.UTTAKSVILKÅR);
        }
            ...
            if (ref.fagsakYtelseType() == FagsakYtelseType.FORELDREPENGER) {
                if (har80Dekningsgrad(ref)) {
                    FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.DEKNINGSGRAD,
                        "barnets dødsdato", grunnlag1.getId(), grunnlag2.getId());
        ...
        if (personstatusUnntattDødEndret) {
            leggTilBasertPåSTP(grunnlag1.getId(), grunnlag2.getId(), startpunkter, poDiff.erPersonstatusEndretForSøkerPeriode(påSkjæringstidpunkt),
                "personstatus");
        }
        ...
        if (barnBorteEndringIdentifiserer.erEndret(ref)) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.SØKERS_RELASJON_TIL_BARNET,
                "barn fjernet fra TPS", grunnlag1.getId(), grunnlag2.getId());
        ...
        if (startpunkter.isEmpty()) {
            ...
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UDEFINERT,
                "personopplysning - andre endringer", g1, g2);
    ...
    private boolean har80Dekningsgrad(BehandlingReferanse referanse) {
        return dekningsgradTjeneste.finnGjeldendeDekningsgradHvisEksisterer(referanse).filter(Dekningsgrad._80::equals).isPresent();
    }
    ...
    private void leggTilBasertPåSTP(Long g1Id, Long g2Id, List<StartpunktType> startpunkter, boolean endretFørStp, String loggMelding) {
        if (endretFørStp) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.INNGANGSVILKÅR_MEDLEMSKAP,
                loggMelding, g1Id, g2Id);
            startpunkter.add(StartpunktType.INNGANGSVILKÅR_MEDLEMSKAP);
        } else {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UTTAKSVILKÅR, loggMelding,
                g1Id, g2Id);
            startpunkter.add(StartpunktType.UTTAKSVILKÅR);
        }
    }
    ...
    private void leggTilForRelasjoner(Long g1Id, Long g2Id, PersonopplysningGrunnlagDiff poDiff, List<StartpunktType> startpunkter) {
        if (poDiff.erRelasjonerEndretSøkerAntallBarn()) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UDEFINERT,
                "personopplysning - relasjon på grunn av fødsel", g1Id, g2Id);
            startpunkter.add(StartpunktType.UDEFINERT);
        }
        if (poDiff.erRelasjonerEndretForSøkerUtenomNyeBarn()) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.SØKERS_RELASJON_TIL_BARNET,
                "personopplysning - brukers relasjoner annet enn fødsel", g1Id, g2Id);
            startpunkter.add(StartpunktType.SØKERS_RELASJON_TIL_BARNET);
        }
        if (poDiff.erRelasjonerEndretForEksisterendeBarn()) {
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.SØKERS_RELASJON_TIL_BARNET,
                "personopplysning - barns relasjoner annet enn fødsel", g1Id, g2Id);
            startpunkter.add(StartpunktType.SØKERS_RELASJON_TIL_BARNET);
        }
        if (poDiff.erRelasjonerBostedEndretForSøkerUtenomNyeBarn()) {
            ...
            FellesStartpunktUtlederLogger.loggEndringSomFørteTilStartpunkt(this.getClass().getSimpleName(), StartpunktType.UTTAKSVILKÅR,
                "personopplysning - relasjoner bosted eller ektefelle", g1Id, g2Id);
            startpunkter.add(StartpunktType.UTTAKSVILKÅR);
        }
    }
