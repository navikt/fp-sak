...
public class MorsAktivitetInnhenter {
    ...
    public void innhentMorsAktivitet(Behandling behandling) {
        ...
        var aktivitetsGrunnlag = aktivitetskravArbeidRepository.hentGrunnlag(behandlingId);

        if (perioderAktivitetskravArbeid.isEmpty() && aktivitetsGrunnlag.isEmpty()) {
            return;
        }

        if (aktivitetsGrunnlag.isPresent() && perioderAktivitetskravArbeid.isEmpty()) {
            aktivitetskravArbeidRepository.fjernGrunnlag(aktivitetsGrunnlag.get());
            return;
        }
        ...
        var morAktvitet = finnMorsAktivitet(behandling, perioderAktivitetskravArbeid, annenPartAktørId);
        if (morAktvitet == null) {
    ...
    public MorAktivitet finnMorsAktivitet(Behandling behandling,
        ...
        var fraDato = minsteFraDatoMinusIntervall(perioderAktivitetskravArbeid);
        var tilDato = hentHøyesteFraDatoPlusIntervall(perioderAktivitetskravArbeid);

        LOG.info("MorsAktivitetInnhenter: Henter mors aktivitet for behandlingId: {} for periode:{} - {}", behandling.getId(), fraDato, tilDato);
        ...
        var perioderBuilder = lagAktivitetskravPerioderBuilder(mapAvOrgnrOgAvtaler, fraDato, tilDato);

        return new MorAktivitet(fraDato, tilDato, perioderBuilder);
    }
    ...
    private static List<OppgittPeriodeEntitet> hentPerioderMedAktivitetskrav(YtelseFordelingAggregat ytelseaggregat) {
        return ytelseaggregat.getGjeldendeFordeling()
            .getPerioder()
            .stream()
            .filter(p -> UttakPeriodeType.FELLESPERIODE.equals(p.getPeriodeType()) && MorsAktivitet.ARBEID.equals(p.getMorsAktivitet()))
            .toList();
    }
