...
public class UtsettelseCore2021 {
    ...
    public static LocalDate førsteUttaksDatoForBeregning(RelasjonsRolleType rolle,
        ...
        if (!gjeldendeFH.getGjelderFødsel() || !RelasjonsRolleType.MORA.equals(rolle)) {
            ...
            var uttaksdato = gjeldendeFH.getSkjæringstidspunkt() != null && førsteUttaksdato.isBefore(
                gjeldendeFH.getSkjæringstidspunkt()) ? gjeldendeFH.getSkjæringstidspunkt() : førsteUttaksdato;
    ...
    public static Optional<LocalDate> finnFørsteDatoFraSøknad(Optional<OppgittFordelingEntitet> oppgittFordeling, boolean kreverSammenhengendeUttak) {
        return oppgittFordeling.map(OppgittFordelingEntitet::getPerioder)
    ...
    private static boolean frittUttakErPeriodeMedUttak(UttakResultatPeriodeEntitet periode) {
        return SØKNADSFRIST.equals(periode.getResultatÅrsak()) || periode.getAktiviteter()
            .stream()
            .anyMatch(a -> a.getUtbetalingsgrad().harUtbetaling() && a.getTrekkdager().merEnn0());
    }
    ...
    private static boolean erAllePerioderAvslåttOgIngenAvslagPgaSøknadsfrist(List<UttakResultatPeriodeEntitet> uttakResultatPerioder) {
        return uttakResultatPerioder.stream()
            .allMatch(ut -> PeriodeResultatType.AVSLÅTT.equals(ut.getResultatType()) && !SØKNADSFRIST.equals(ut.getResultatÅrsak()));
    }
