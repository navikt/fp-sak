...
public class AvklarLøpendeOmsorgAksjonspunktUtleder {
    ...
    public Optional<AksjonspunktDefinisjon> utledAksjonspunktFor(UttakInput input) {
        ...
        if (BehandlingType.REVURDERING.equals(ref.behandlingType()) && ytelsesFordelingRepository.hentAggregatHvisEksisterer(ref.behandlingId())
            .filter(a -> Boolean.TRUE.equals(a.getOverstyrtOmsorg()))
            .isPresent()) {
            return Optional.empty();
        }
