...
class PeriodeSplitter {
    ...
    static List<OppgittPeriodeEntitet> splittPeriodeMotVenstre(List<OppgittPeriodeEntitet> ikkeFlyttbarePerioder,
        ...
        for (var i = ikkeFlyttbarePerioder.size() - 1; i >= 0; i--) {
            ...
            var antallIkkeFlyttbareDager =
                ChronoUnit.DAYS.between(ikkeFlyttbarPeriode.getFom(), ikkeFlyttbarPeriode.getTom()) + 1;
            ...
            if (nyFom.isBefore(ikkeFlyttbarPeriode.getTom()) && nyTom.isAfter(ikkeFlyttbarPeriode.getTom())) {
                ...
                splittedePerioder.add(0, OppgittPeriodeBuilder.fraEksisterende(periode)
                    ...
                    .build());
                var restDager = ChronoUnit.DAYS.between(nyFom, nyTom) + 1 - (
                    ChronoUnit.DAYS.between(ikkeFlyttbarPeriode.getTom().plusDays(1), nyTom) + 1);
        ...
        for (var ikkeFlyttbarPeriode : ikkeFlyttbarePerioder) {
            ...
            if (nyFom.isBefore(ikkeFlyttbarPeriode.getFom()) && nyTom.isAfter(ikkeFlyttbarPeriode.getFom())) {
                ...
                splittedePerioder.add(OppgittPeriodeBuilder.fraEksisterende(periode)
                    ...
                    .build());
                var restDager = ChronoUnit.DAYS.between(nyFom, nyTom) + 1 - (
                    ChronoUnit.DAYS.between(nyFom, ikkeFlyttbarPeriode.getFom().minusDays(1)) + 1);
