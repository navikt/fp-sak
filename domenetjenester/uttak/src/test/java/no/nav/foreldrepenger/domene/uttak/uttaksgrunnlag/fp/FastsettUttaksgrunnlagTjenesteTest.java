...
class FastsettUttaksgrunnlagTjenesteTest {
    ...
    private final EndringsdatoFørstegangsbehandlingUtleder endringsdatoUtleder = new EndringsdatoFørstegangsbehandlingUtleder(
    ...
    void skal_kopiere_søknadsperioder_fra_forrige_behandling_hvis_forrige_behandling_ikke_har_uttaksresultat() {
        ...
        var periode = ny().medPeriode(førsteUttaksdato, LocalDate.now().plusDays(10)).medPeriodeType(FELLESPERIODE).build();
        ...
        assertThat(resultat.getGjeldendeFordeling().getPerioder()).isEqualTo(forrigeBehandlingFordeling.getOppgittFordeling().getPerioder());
    ...
    void skal_lagre_opprinnelig_endringsdato() {
        ...
        var behandling = ScenarioFarSøkerForeldrepenger.forFødsel().medFordeling(fordeling).lagre(repositoryProvider);
    ...
    void skal_beholde_overstyrt_dersom_uendret_justering() {
        ...
        var overstyrtFordeling = new OppgittFordelingEntitet(List.of(fraEksisterende(periode).medPeriodeType(MØDREKVOTE).build()), true);

        var avklarteUttakDatoer = new AvklarteUttakDatoerEntitet.Builder().medOpprinneligEndringsdato(førsteUttaksdato).build();
    ...
    void skal_fjerne_overstyrt_dersom_endret_justering() {
        ...
        var førsteUttaksdato = LocalDate.now();
        var periode = ny().medPeriode(førsteUttaksdato, LocalDate.now().plusDays(10)).medPeriodeType(FELLESPERIODE).build();
        var periodeMK = ny().medPeriode(førsteUttaksdato, LocalDate.now().plusDays(10)).medPeriodeType(MØDREKVOTE).build();
        var fordeling = new OppgittFordelingEntitet(List.of(periode), true);
        ...
        var avklarteUttakDatoer = new AvklarteUttakDatoerEntitet.Builder().medOpprinneligEndringsdato(LocalDate.of(2020, 10, 10)).build();
        var behandling = ScenarioFarSøkerForeldrepenger.forFødsel()
            .medFordeling(fordeling)
            .medJustertFordeling(justertFordeling)
            .medAvklarteUttakDatoer(avklarteUttakDatoer)
    ...
    void skal_ikke_forskyve_søknadsperioder_hvis_både_termin_og_fødsel_er_oppgitt_i_søknad() {
        ...
        var periode = ny().medPeriode(søknadFom, søknadFom.plusDays(10)).medPeriodeType(FELLESPERIODE).build();
        var fordeling = new OppgittFordelingEntitet(List.of(periode), true);
    ...
    void skal_ikke_forskyve_søknadsperioder_hvis_ny_førstegangssøknad_med_endret_termindato() {
        ...
        var fordeling1 = new OppgittFordelingEntitet(
            List.of(ny().medPeriode(LocalDate.of(2024, 4, 15), LocalDate.of(2024, 5, 3)).medPeriodeType(FORELDREPENGER_FØR_FØDSEL).build(),
                ny().medPeriode(LocalDate.of(2024, 5, 6), LocalDate.of(2024, 9, 13)).medPeriodeType(MØDREKVOTE).build(),
                ny().medPeriode(LocalDate.of(2024, 9, 16), LocalDate.of(2025, 1, 17)).medPeriodeType(FELLESPERIODE).build()), true);
        var uttaksperiodeFørstegang = new UttakResultatPeriodeEntitet.Builder(termindato1, termindato1.plusWeeks(10)).medResultatType(
        ...
        var førstegangsbehandling = ScenarioMorSøkerForeldrepenger.forFødsel()
            ...
            .lagre(repositoryProvider);
        var fordeling2 = new OppgittFordelingEntitet(
            List.of(ny().medPeriode(LocalDate.of(2024, 4, 15), LocalDate.of(2024, 4, 26)).medPeriodeType(FORELDREPENGER_FØR_FØDSEL).build(),
                ...
                ny().medPeriode(LocalDate.of(2024, 11, 15), LocalDate.of(2025, 2, 11)).medPeriodeType(MØDREKVOTE).build()), true);
        ...
        var fpGrunnlag = new ForeldrepengerGrunnlag().medOriginalBehandling(
                new OriginalBehandling(førstegangsbehandling.getId(), new FamilieHendelser().medSøknadHendelse(familieHendelseFørstegangsbehandling)))
            .medFamilieHendelser(
                new FamilieHendelser().medSøknadHendelse(familieHendelseRevurdering).medBekreftetHendelse(familieHendelseRevurdering));
    ...
    void sammenhengende_uttak_skal_fjerne_oppholdsperioder_på_slutten_av_fordelingen() {
        var søknadFom = LocalDate.of(2019, 7, 31);
        var periode1 = ny().medPeriode(søknadFom, søknadFom.plusDays(10)).medPeriodeType(FELLESPERIODE).build();
        var opphold1 = ny().medÅrsak(OppholdÅrsak.FEDREKVOTE_ANNEN_FORELDER)
            .medPeriode(periode1.getTom().plusDays(1), periode1.getTom().plusDays(10))
            .build();
        var periode2 = ny().medPeriode(opphold1.getTom().plusDays(1), opphold1.getTom().plusDays(10)).medPeriodeType(FELLESPERIODE).build();
        var opphold2 = ny().medÅrsak(OppholdÅrsak.FEDREKVOTE_ANNEN_FORELDER)
            .medPeriode(periode2.getTom().plusDays(1), periode2.getTom().plusDays(10))
            .build();
        var opphold3 = ny().medÅrsak(OppholdÅrsak.KVOTE_FELLESPERIODE_ANNEN_FORELDER)
    ...
    void adopsjon_uten_justering_men_oppdatere_mottatt_dato() {
        ...
        var søknadsperiode = ny().medPeriodeType(MØDREKVOTE)
        ...
        var søknadFamilieHendelse = FamilieHendelse.forAdopsjonOmsorgsovertakelse(søknadFom, List.of(new Barn(null)), 1, søknadFom.minusWeeks(2),
            false);
        var fpGrunnlag = new ForeldrepengerGrunnlag().medFamilieHendelser(new FamilieHendelser().medSøknadHendelse(søknadFamilieHendelse));
        tjeneste.fastsettUttaksgrunnlag(lagInput(behandling, fpGrunnlag));
    ...
    void tillater_overlappende_perioder_men_skal_ikke_justeres_bare_slås_sammen_hvis_mulig() {
        ...
        var mødrekvote = ny().medPeriode(fødselsdato, fødselsdato.plusDays(10)).medPeriodeType(MØDREKVOTE).build();
        // overlapper med mødrekvoten
        var fellesperiode = ny().medPeriode(fødselsdato.plusDays(5), fødselsdato.plusDays(20)).medPeriodeType(FELLESPERIODE).build();
    ...
    void fjerner_perioder_før_endringsdato_i_revuderinger() {
        ...
        var mødrekvote1 = ny().medPeriode(fødselsdato, fødselsdato.plusWeeks(3).minusDays(1)).medPeriodeType(MØDREKVOTE).build();
        var mødrekvote2 = ny().medPeriode(fødselsdato.plusWeeks(3), fødselsdato.plusWeeks(10)).medPeriodeType(MØDREKVOTE).build();
        var fordeling = new OppgittFordelingEntitet(List.of(mødrekvote1, mødrekvote2), true);
        var uttaksperiode = new UttakResultatPeriodeEntitet.Builder(mødrekvote1.getFom(), mødrekvote2.getTom()).medResultatType(
            PeriodeResultatType.INNVILGET, PeriodeResultatÅrsak.KVOTE_ELLER_OVERFØRT_KVOTE).build();
        var uttak = new UttakResultatPerioderEntitet().leggTilPeriode(uttaksperiode);
        var førstegangsbehandlingScenario = ScenarioMorSøkerForeldrepenger.forFødsel().medUttak(uttak).medFordeling(fordeling);
        var behandling = førstegangsbehandlingScenario.lagre(repositoryProvider);
