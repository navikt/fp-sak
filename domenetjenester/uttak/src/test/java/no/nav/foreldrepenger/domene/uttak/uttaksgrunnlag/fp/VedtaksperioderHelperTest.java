...
class VedtaksperioderHelperTest {
    ...
    void skal_lage_en_klippet_vedtaksperiode_av_tidligere_uttak_fra_endringsdato() {
        ...
        uttakResultatPerioderEntitet.leggTilPeriode(
            nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato, fødselsdato.plusWeeks(6).minusDays(1), UttakPeriodeType.MØDREKVOTE));
        uttakResultatPerioderEntitet.leggTilPeriode(
        ...
        var uttakResultatEntitet = new UttakResultatEntitet.Builder(mock(Behandlingsresultat.class)).medOpprinneligPerioder(
            uttakResultatPerioderEntitet).build();

        var perioder = VedtaksperioderHelper.opprettOppgittePerioder(uttakResultatEntitet, List.of(), fødselsdato.plusWeeks(10), false);
    ...
    void skal_lage_vedtaksperioder_av_tidligere_uttak() {
        ...
        var perioder = VedtaksperioderHelper.opprettOppgittePerioder(uttakResultatEntitet, List.of(), fødselsdato.minusWeeks(3), false);
    ...
    void skal_ikke_lage_vedtaksperioder_av_tidligere_uttak_og_endringsdato_lik_første_søknadsdato() {
        ...
        uttakResultatPerioderEntitet.leggTilPeriode(nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1),
            UttakPeriodeType.FORELDREPENGER_FØR_FØDSEL));
        uttakResultatPerioderEntitet.leggTilPeriode(
            nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato, fødselsdato.plusWeeks(12).minusDays(1), UttakPeriodeType.MØDREKVOTE));
        var uttakResultatEntitet = new UttakResultatEntitet.Builder(mock(Behandlingsresultat.class)).medOpprinneligPerioder(
            uttakResultatPerioderEntitet).build();
        ...
        //Kjør tjeneste for å opprette søknadsperioder for revurdering.
        var perioder = VedtaksperioderHelper.opprettOppgittePerioder(uttakResultatEntitet, List.of(fp), fødselsdato.plusWeeks(10), false);
    ...
    void skal_lage_vedtaksperioder_av_deler_av_tidligere_uttak_og_flett_inn_endringsøknad() {
        ...
        //Kjør tjeneste for å opprette søknadsperioder for revurdering.
        var perioder = VedtaksperioderHelper.opprettOppgittePerioder(uttakResultatEntitet, List.of(mk), fødselsdato.plusWeeks(10), false);
    ...
    void søknadsperiode_start_overlapp_med_sluttdato_på_uttak() {
        ...
        uttakResultatPerioderEntitet.leggTilPeriode(nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1),
            UttakPeriodeType.FORELDREPENGER_FØR_FØDSEL));
        uttakResultatPerioderEntitet.leggTilPeriode(
            nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato, fødselsdato.plusWeeks(3).minusDays(1), UttakPeriodeType.MØDREKVOTE));
        var uttakResultatEntitet = new UttakResultatEntitet.Builder(mock(Behandlingsresultat.class)).medOpprinneligPerioder(
            uttakResultatPerioderEntitet).build();
    ...
    void uttakperioder_avslått_pga_overlapp_med_annenpart_og_med_null_trekk_ikke_kopieres_med_i_neste_behandling() {
        ...
        var avlått = new UttakResultatPeriodeEntitet.Builder(fødselsdato.plusWeeks(12), fødselsdato.plusWeeks(16).minusDays(1)).medResultatType(
                PeriodeResultatType.AVSLÅTT, PeriodeResultatÅrsak.DEN_ANDRE_PART_OVERLAPPENDE_UTTAK_IKKE_SØKT_INNVILGET_SAMTIDIG_UTTAK)
            .medPeriodeSoknad(periodeSøknad)
            .build();

        avlått.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(avlått, opprettArbeidstakerUttakAktivitet("orgnr"))
    ...
    void uttakperioder_avslått_pga_overlapp_med_utsettelse_hos_annenpart_og_med_null_trekk_ikke_kopieres_med_i_neste_behandling() {
        ...
        var periodeSøknad = new UttakResultatPeriodeSøknadEntitet.Builder().medUttakPeriodeType(UttakPeriodeType.FELLESPERIODE).build();
        var avlått = new UttakResultatPeriodeEntitet.Builder(fødselsdato.plusWeeks(12), fødselsdato.plusWeeks(16).minusDays(1)).medResultatType(
                PeriodeResultatType.AVSLÅTT, PeriodeResultatÅrsak.DEN_ANDRE_PART_HAR_OVERLAPPENDE_UTTAKSPERIODER_SOM_ER_INNVILGET_UTSETTELSE)
            .medPeriodeSoknad(periodeSøknad)
            .build();
    ...
    void konvertererIkkeGodkjentUtsettelse() {
        ...
        var periodeEntitet = new UttakResultatPeriodeEntitet.Builder(fom, tom).medUtsettelseType(UttakUtsettelseType.FERIE)
        ...
        periodeEntitet.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(periodeEntitet, opprettArbeidstakerUttakAktivitet("orgnr"))
            .medTrekkonto(UttakPeriodeType.FELLESPERIODE)
    ...
    void konvertererUttakMedGraderingSomArbeidstaker() {
        ...
        var periodeEntitet = new UttakResultatPeriodeEntitet.Builder(fom, tom).medResultatType(PeriodeResultatType.INNVILGET,
                PeriodeResultatÅrsak.UKJENT)
            .medGraderingInnvilget(true)
            .medPeriodeSoknad(new UttakResultatPeriodeSøknadEntitet.Builder().medGraderingArbeidsprosent(arbeidstidsprosent)
                .medUttakPeriodeType(UttakPeriodeType.FEDREKVOTE)
                .build())
            .build();

        periodeEntitet.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(periodeEntitet, opprettArbeidstakerUttakAktivitet("orgnr1"))
        ...
        periodeEntitet.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(periodeEntitet, opprettSelvNærUttakAktivitetet())
            .medTrekkonto(UttakPeriodeType.FEDREKVOTE)
            .medTrekkdager(new Trekkdager(5))
    ...
    void konverterer_uttak_inkludert_samtidig_uttak_og_flerbarnsdager() {
        ...
        var søknadPeriode = new UttakResultatPeriodeSøknadEntitet.Builder().medUttakPeriodeType(UttakPeriodeType.FEDREKVOTE)
    ...
    void konvertererUttakMedGraderingSomSelvstendigNæringsdrivende() {
        ...
        periodeEntitet.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(periodeEntitet, opprettArbeidstakerUttakAktivitet("orgnr2"))
            .medTrekkonto(UttakPeriodeType.FORELDREPENGER)
            .medTrekkdager(new Trekkdager(5))
            .medUtbetalingsgrad(new Utbetalingsgrad(100))
            .medArbeidsprosent(BigDecimal.ZERO)
    ...
    void skal_ikke_ta_med_uttak_periode_som_ikke_er_knyttet_til_søknadsperiode() {
        var uttakResultatPerioderEntitet = new UttakResultatPerioderEntitet();
        uttakResultatPerioderEntitet.leggTilPeriode(
            nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1), UttakPeriodeType.FORELDREPENGER_FØR_FØDSEL,
                false));
        uttakResultatPerioderEntitet.leggTilPeriode(
            nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato, fødselsdato.plusWeeks(6).minusDays(1), UttakPeriodeType.MØDREKVOTE, true));
    ...
    void skal_lage_en_vedtaksperiode_av_uttaksresultatperiode_på_en_dag() {
        var uttakResultatPerioderEntitet = new UttakResultatPerioderEntitet();
        var uttakResultatPeriode = nyPeriode(PeriodeResultatType.INNVILGET, fødselsdato, fødselsdato, UttakPeriodeType.MØDREKVOTE);
    ...
    void skalKonvertereSamtidigUttak() {
        ...
        var periodeSøknad = new UttakResultatPeriodeSøknadEntitet.Builder().medSamtidigUttak(true)
            .medSamtidigUttaksprosent(SamtidigUttaksprosent.TEN)
    ...
    void skalHåndtereSamtidigUttaksprosentNull() {
        ...
        var periodeSøknad = new UttakResultatPeriodeSøknadEntitet.Builder().medSamtidigUttaksprosent(null)
            ...
            .build();
        var periodeEntitet = new UttakResultatPeriodeEntitet.Builder(fom, tom).medResultatType(PeriodeResultatType.INNVILGET,
            PeriodeResultatÅrsak.UKJENT).medPeriodeSoknad(periodeSøknad).build();
        periodeEntitet.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(periodeEntitet, opprettSelvNærUttakAktivitetet())
            .medTrekkonto(UttakPeriodeType.FORELDREPENGER)
            .medTrekkdager(new Trekkdager(2))
            .medArbeidsprosent(BigDecimal.TEN)
    ...
    void skalKonvertereOverføringÅrsak() {
        var uttaksperiode = new UttakResultatPeriodeEntitet.Builder(LocalDate.of(2018, Month.JULY, 3),
            LocalDate.of(2018, Month.JULY, 10)).medResultatType(PeriodeResultatType.INNVILGET, PeriodeResultatÅrsak.KVOTE_ELLER_OVERFØRT_KVOTE)
            ...
            .medPeriodeSoknad(new UttakResultatPeriodeSøknadEntitet.Builder().medMottattDato(LocalDate.of(2017, 1, 1))
                ...
                .build())
            .build();
        uttaksperiode.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(uttaksperiode, opprettArbeidstakerUttakAktivitet("123"))
            .medTrekkonto(UttakPeriodeType.FEDREKVOTE)
            .medTrekkdager(new Trekkdager(2))
            .medArbeidsprosent(BigDecimal.TEN)
            .build());
    ...
    void skalKonvertereMottattDato() {
        ...
        var uttaksperiode = new UttakResultatPeriodeEntitet.Builder(LocalDate.of(2018, Month.JULY, 3),
            LocalDate.of(2018, Month.JULY, 10)).medResultatType(PeriodeResultatType.INNVILGET, PeriodeResultatÅrsak.KVOTE_ELLER_OVERFØRT_KVOTE)
            .medPeriodeSoknad(
                new UttakResultatPeriodeSøknadEntitet.Builder().medUttakPeriodeType(UttakPeriodeType.FEDREKVOTE).medMottattDato(mottattDato).build())
            .build();
        uttaksperiode.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(uttaksperiode, opprettArbeidstakerUttakAktivitet("123"))
            .medTrekkonto(UttakPeriodeType.FEDREKVOTE)
            .medTrekkdager(new Trekkdager(2))
    ...
    void skalIkkeKopiereMedDokumentasjonsVurderingHvisInnvilgetRedusertUttakPgaMorDelvisArbeid() {
        var uttaksperiode1 = new UttakResultatPeriodeEntitet.Builder(LocalDate.of(2018, Month.JULY, 3),
            LocalDate.of(2018, Month.JULY, 10)).medResultatType(PeriodeResultatType.INNVILGET,
                PeriodeResultatÅrsak.FORELDREPENGER_FELLESPERIODE_TIL_FAR)
            .medPeriodeSoknad(
                new UttakResultatPeriodeSøknadEntitet.Builder().medDokumentasjonVurdering(DokumentasjonVurdering.MORS_AKTIVITET_GODKJENT)
                    .medUttakPeriodeType(UttakPeriodeType.FELLESPERIODE)
                    .medMorsAktivitet(MorsAktivitet.ARBEID)
                    .build())
            .build();
        uttaksperiode1.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(uttaksperiode1, opprettArbeidstakerUttakAktivitet("123"))
            .medTrekkonto(UttakPeriodeType.FELLESPERIODE)
            .medTrekkdager(new Trekkdager(2))
            //Mor 50% i arbeid
            .medUtbetalingsgrad(new Utbetalingsgrad(50))
        ...
        var uttaksperiode2 = new UttakResultatPeriodeEntitet.Builder(LocalDate.of(2020, Month.JULY, 3),
            LocalDate.of(2020, Month.JULY, 10)).medResultatType(PeriodeResultatType.INNVILGET,
                PeriodeResultatÅrsak.FORELDREPENGER_FELLESPERIODE_TIL_FAR)
            .medPeriodeSoknad(
                new UttakResultatPeriodeSøknadEntitet.Builder().medDokumentasjonVurdering(DokumentasjonVurdering.MORS_AKTIVITET_GODKJENT)
                    .medUttakPeriodeType(UttakPeriodeType.FELLESPERIODE)
                    ...
                    .build())
            .build();
        uttaksperiode2.leggTilAktivitet(UttakResultatPeriodeAktivitetEntitet.builder(uttaksperiode2, opprettArbeidstakerUttakAktivitet("123"))
            .medTrekkonto(UttakPeriodeType.FELLESPERIODE)
            .medTrekkdager(new Trekkdager(2))
            //Mor 100% i arbeid
        ...
        assertThat(konvertetPeriode2.getDokumentasjonVurdering()).isEqualTo(
            uttaksperiode2.getPeriodeSøknad().orElseThrow().getDokumentasjonVurdering());
    }
    ...
    private UttakAktivitetEntitet opprettSelvNærUttakAktivitetet() {
        return new UttakAktivitetEntitet.Builder().medUttakArbeidType(UttakArbeidType.SELVSTENDIG_NÆRINGSDRIVENDE).build();
    }

    private UttakResultatPeriodeEntitet nyPeriode(PeriodeResultatType resultat, LocalDate fom, LocalDate tom, UttakPeriodeType stønadskontoType) {
    ...
    private UttakResultatPeriodeEntitet nyPeriode(PeriodeResultatType resultat,
        ...
        var uttakAktivitet = new UttakAktivitetEntitet.Builder().medArbeidsforhold(Arbeidsgiver.virksomhet(arbeidsgiverOrgnr),
        ...
        if (knyttTilSøknadsperiode) {
            var periodeSøknad = new UttakResultatPeriodeSøknadEntitet.Builder().medUttakPeriodeType(stønadskontoType)
