...
public class OppdragskontrollTestVerktøy {
    ...
    public static List<Oppdragslinje150> getOppdragslinje150MedKlassekode(List<Oppdragslinje150> opp150Liste, KodeKlassifik klassekode) {
        return opp150Liste.stream().filter(opp150 -> opp150.getKodeKlassifik().equals(klassekode)).toList();
    }

    public static Oppdrag110 getOppdrag110ForBruker(List<Oppdrag110> oppdrag110Liste) {
        return oppdrag110Liste.stream().filter(oppdrag110 -> KodeFagområde.FP.equals(oppdrag110.getKodeFagomrade())).findFirst().get();
    }
    ...
    public static void verifiserKjedingForOppdragslinje150(List<Oppdragslinje150> originaltOppdr150ListeAT,
                                                           List<Oppdragslinje150> originaltOppdr150ListeFL) {
    ...
    public static void verifiserOppdr150SomAndelerSlåSammen(Oppdragskontroll originaltOppdrag, Oppdragskontroll revurderingOppdrag) {
        ...
        var originaltOppdr150ListeBruker = originaltoppdr150Liste.stream()
            .filter(opp150 -> opp150.getOppdrag110().getKodeFagomrade().equals(KodeFagområde.FP))
        ...
        var revurderingOppdr150ListeBruker = revurderingOppdr150Liste.stream()
            .filter(opp150 -> opp150.getOppdrag110().getKodeFagomrade().equals(KodeFagområde.FP))
            .filter(opp150 -> !opp150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER))
            .filter(opp150 -> opp150.getKodeEndringLinje().equals(KodeEndringLinje.NY))
            .filter(opp150 -> opp150.getDatoVedtakFom().equals(LocalDate.now().plusDays(8)))
            .toList();
    ...
    public static void verifiserOppdr150MedNyKlassekode(List<Oppdragslinje150> opp150RevurdListe) {
        var opp150Liste = opp150RevurdListe.stream()
            .filter(oppdr150 -> !oppdr150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER) && !oppdr150.getKodeKlassifik()
                .equals(KodeKlassifik.FPF_FERIEPENGER_AG))
    ...
    public static void verifiserDelYtelseOgFagsystemIdForEnKlassekode(List<Oppdragslinje150> opp150RevurderingListe,
                                                                      List<Oppdragslinje150> opp150OriginalListe) {
        ...
        assertThat(opp150OriginalListe).anySatisfy(
            opp150Original -> assertThat(opp150Original.getDelytelseId()).isEqualTo(førsteOpp150IRevurderingKjede.getRefDelytelseId()));
    }

    public static void verifiserDelYtelseOgFagsystemIdForFlereKlassekode(List<Oppdragslinje150> opp150RevurderingListe,
                                                                         List<Oppdragslinje150> opp150OriginalListe) {
        opp150OriginalListe.removeIf(opp150 -> opp150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER) || opp150.getKodeKlassifik()
            .equals(KodeKlassifik.FPF_FERIEPENGER_AG));
        ...
        klassekodeListe.forEach(kode -> {
            if (opp150RevurderingListe.stream().anyMatch(opp150 -> opp150.getKodeKlassifik().equals(kode))) {
                var førsteOpp150IKjede = opp150RevurderingListe.stream()
                    .filter(opp150 -> opp150.getKodeKlassifik().equals(kode) && !opp150.gjelderOpphør())
                    .min(Comparator.comparing(Oppdragslinje150::getDatoVedtakFom))
                    .get();
                assertThat(opp150OriginalListe).anySatisfy(
                    opp150 -> assertThat(opp150.getDelytelseId()).isEqualTo(førsteOpp150IKjede.getRefDelytelseId()));
            }
        });
    }

    public static List<Oppdragslinje150> getOppdragslinje150Liste(Oppdragskontroll oppdragskontroll) {
        return oppdragskontroll.getOppdrag110Liste().stream().flatMap(oppdrag110 -> oppdrag110.getOppdragslinje150Liste().stream()).toList();
    }

    public static boolean erOpp150ForFeriepenger(Oppdragslinje150 opp150) {
        return opp150.getKodeKlassifik().equals(KodeKlassifik.FPF_FERIEPENGER_AG) || opp150.getKodeKlassifik()
            .equals(KodeKlassifik.FERIEPENGER_BRUKER);
    }
    ...
    public static void verifiserAvstemming(Oppdragskontroll oppdragRevurdering) {
        var avstemmingRevurdList = oppdragRevurdering.getOppdrag110Liste().stream().map(Oppdrag110::getAvstemming).toList();
    ...
    public static void verifiserGrad(List<Oppdragslinje150> opp150RevurderingList, Oppdragskontroll originaltOppdrag) {
        ...
        for (var opp150Revurdering : opp150RevurderingList) {
            ...
            var originaltOpp150 = originaltOpp150Liste.stream()
                .filter(opp150 -> opp150.getDelytelseId().equals(opp150Revurdering.getDelytelseId()))
                .findFirst()
                .orElse(null);
    ...
    public static void verifiserRefusjonInfo156(List<Oppdrag110> opp110RevurderingList, Oppdragskontroll originaltOppdrag) {
        ...
        var opp150RevurderingList = opp110RevurderingList.stream()
            .filter(opp110 -> opp110.getKodeFagomrade().equals(KodeFagområde.FPREF))
        ...
        for (var opp150Revurdering : opp150RevurderingList) {
            var originaltOpp150 = originaltOpp150Liste.stream()
                .filter(opp150 -> opp150.getDelytelseId().equals(opp150Revurdering.getRefDelytelseId()))
                .findFirst()
                .orElse(null);
    ...
    public static void verifiserOpphørsdatoen(Oppdragskontroll originaltOppdrag, Oppdragskontroll oppdragRevurdering) {
        var originaltOppdr150Liste = originaltOppdrag.getOppdrag110Liste()
            .stream()
            .flatMap(oppdrag110 -> oppdrag110.getOppdragslinje150Liste().stream())
            .toList();
        var oppdr150OpphørtListe = oppdragRevurdering.getOppdrag110Liste()
            .stream()
            .flatMap(oppdrag110 -> oppdrag110.getOppdragslinje150Liste().stream())
            .filter(opp150 -> opp150.gjelderOpphør() && !opp150.getKodeKlassifik().equals(KodeKlassifik.FPF_REFUSJON_AG))
            .toList();
        for (var opp150Opphørt : oppdr150OpphørtListe) {
            var klassekode = opp150Opphørt.getKodeKlassifik();
            var førsteDatoVedtakFom = originaltOppdr150Liste.stream()
                .filter(opp150 -> opp150.getKodeKlassifik().equals(klassekode))
                .min(Comparator.comparing(Oppdragslinje150::getDatoVedtakFom))
    ...
    public static LocalDate finnFørsteDatoVedtakFom(List<Oppdragslinje150> originaltOpp150Liste, Oppdragslinje150 originaltOpp150) {
        if (originaltOpp150.getOppdrag110().getKodeFagomrade().equals(KodeFagområde.FP)) {
            return originaltOpp150Liste.stream()
                .filter(opp150 -> !opp150.getKodeKlassifik().equals(KodeKlassifik.FPF_REFUSJON_AG) && !opp150.getKodeKlassifik()
                    .equals(KodeKlassifik.FPF_FERIEPENGER_AG))
                .min(Comparator.comparing(Oppdragslinje150::getDatoVedtakFom))
                .map(Oppdragslinje150::getDatoVedtakFom)
                .get();
        ...
        return originaltOpp150Liste.stream()
            .filter(opp150 -> opp150.getKodeKlassifik().equals(KodeKlassifik.FPF_REFUSJON_AG))
    ...
    public static boolean opp150MedGradering(Oppdragslinje150 oppdragslinje150) {
        var erBrukerEllerVirksomhet =
            oppdragslinje150.getOppdrag110().getKodeFagomrade().equals(KodeFagområde.FP) || oppdragslinje150.getRefusjonsinfo156()
                .getRefunderesId()
                .equals("00789123456");
        var gjelderFeriepenger = oppdragslinje150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER) || oppdragslinje150.getKodeKlassifik()
            .equals(KodeKlassifik.FPF_FERIEPENGER_AG);
        return erBrukerEllerVirksomhet && !gjelderFeriepenger;
    }


    public static void verifiserOppdr150SomErNy(List<Oppdragslinje150> opp150RevurdListe,
                                                List<Oppdragslinje150> originaltOpp150Liste,
                                                List<Integer> gradering) {
        var opp150NyList = opp150RevurdListe.stream().filter(oppdr150 -> oppdr150.getKodeEndringLinje().equals(KodeEndringLinje.NY)).toList();
        ...
        for (var opp150Ny : opp150NyList) {
            ...
            if (opp150Ny.getRefDelytelseId() != null) {
                assertThat(opp150RevurdListe).anySatisfy(opp150 -> assertThat(opp150.getDelytelseId()).isEqualTo(opp150Ny.getRefDelytelseId()));
    ...
    public static void verifiserOppdr150SomErOpphørt(List<Oppdragslinje150> opp150RevurdListe,
                                                     List<Oppdragslinje150> originaltOpp150Liste,
                                                     boolean medFeriePenger,
                                                     boolean medFlereKlassekode,
                                                     boolean opphFomEtterStp) {
        ...
        for (var opp150Revurd : opp150RevurdListe) {
            ...
            if (medFlereKlassekode) {
                ...
                assertThat(kodeKlassifikRevurderingListe).containsAnyElementsOf(
                    opphFomEtterStp ? Collections.singletonList(KodeKlassifik.FPF_REFUSJON_AG) : kodeKlassifikForrigeListe);
            }
            if (originaltOpp150 != null) {
    ...
    public static List<KodeKlassifik> getKodeklassifikKunForOpp150MedOpph(List<Oppdragslinje150> opp150RevurdListe) {
        return opp150RevurdListe.stream()
            .filter(opp150 -> !opp150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER) && !opp150.getKodeKlassifik()
                .equals(KodeKlassifik.FPF_FERIEPENGER_AG))
     ...
     */
    private static void verifiserOpphørForrigeOppdrag(List<Oppdragslinje150> originaltOpp150Liste,
                                                      List<LocalDate> opphørsdatoVerdierForFeriepenger,
                                                      Oppdragslinje150 opp150Revurd,
                                                      Oppdragslinje150 originaltOpp150) {
    ...
    private static void verifiserFeriepenger(List<Oppdragslinje150> opp150RevurdListe) {
        assertThat(opp150RevurdListe).anySatisfy(opp150 -> assertThat(opp150.getKodeKlassifik()).isEqualTo(KodeKlassifik.FERIEPENGER_BRUKER));
        assertThat(opp150RevurdListe).anySatisfy(opp150 -> assertThat(opp150.getKodeKlassifik()).isEqualTo(KodeKlassifik.FPF_FERIEPENGER_AG));
        assertThat(opp150RevurdListe).anySatisfy(opp150 -> assertThat(opp150.getTypeSats()).isEqualTo(TypeSats.ENG));
        var opp150FeriepgBrukerList = opp150RevurdListe.stream()
            .filter(o150 -> o150.getUtbetalesTilId() != null)
            .filter(opp150 -> opp150.getKodeKlassifik().equals(KodeKlassifik.FERIEPENGER_BRUKER))
            .filter(Oppdragslinje150::gjelderOpphør)
            .toList();
        var opp150ArbeidsgiverList = opp150RevurdListe.stream()
            .filter(opp150 -> opp150.getKodeKlassifik().equals(KodeKlassifik.FPF_FERIEPENGER_AG))
            .toList();
        assertThat(opp150FeriepgBrukerList).anySatisfy(opp150 -> assertThat(opp150.getKodeStatusLinje()).isEqualTo(KodeStatusLinje.OPPH));
        assertThat(opp150ArbeidsgiverList).anySatisfy(opp150 -> assertThat(opp150.getKodeStatusLinje()).isEqualTo(KodeStatusLinje.OPPH));
    }
}
