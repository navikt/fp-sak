...
public class VurderFagsystemFellesUtils {
    ...
    public List<Fagsak> filtrerSakerForBehandlingTema(List<Fagsak> saker, BehandlingTema behandlingTema) {
        ...
        return saker.stream().filter(s -> behandlingTema.erKompatibelMed(this.getBehandlingsTemaForFagsak(s))).toList();
    }

    public List<Fagsak> finnÅpneSaker(List<Fagsak> saker) {
        return saker.stream().filter(Fagsak::erÅpen).filter(s -> FagsakStatus.LØPENDE.equals(s.getStatus()) || harÅpenYtelsesBehandling(s)).toList();
    }
    ...
    private List<Fagsak> harSakMedAvslagGrunnetManglendeDok(List<Fagsak> saker) {
        return saker.stream()
            .filter(s -> mottatteDokumentTjeneste.erSisteYtelsesbehandlingAvslåttPgaManglendeDokumentasjon(s)
                && !mottatteDokumentTjeneste.harFristForInnsendingAvDokGåttUt(s))
            .toList();
    }
    ...
    public Optional<FamilieHendelseEntitet> finnGjeldendeFamilieHendelseSVP(Fagsak fagsak) {
        return behandlingRepository.finnSisteIkkeHenlagteYtelseBehandlingFor(fagsak.getId())
            .flatMap(behandling -> familieHendelseTjeneste.finnAggregat(behandling.getId()).map(FamilieHendelseGrunnlagEntitet::getGjeldendeVersjon));
    }
    ...
    public List<Fagsak> sakerOpprettetInnenIntervall(List<Fagsak> sakerGittYtelseType) {
        ...
        return sakerGittYtelseType.stream().filter(f -> f.getOpprettetTidspunkt() != null && f.getOpprettetTidspunkt().isAfter(sammenlign)).toList();
    }
    ...
    public boolean harSakOpprettetInnenIntervallForIM(List<Fagsak> sakerGittYtelseType, LocalDate referanseDato) {
        ...
        return sakerGittYtelseType.stream().anyMatch(f -> f.getOpprettetTidspunkt() != null && f.getOpprettetTidspunkt().isAfter(sammenlign));
    }
    ...
    public boolean harBehandlingTilkjentRundtIM(Behandling behandling, LocalDate referanseDato) {
        var tilkjent = beregningsresultatRepository.hentUtbetBeregningsresultat(behandling.getId())
            ...
            .orElse(List.of());
        var min = tilkjent.stream()
            .map(BeregningsresultatPeriode::getBeregningsresultatPeriodeFom)
            .min(Comparator.naturalOrder())
            .orElse(Tid.TIDENES_ENDE)
            .minusWeeks(3);
        var max = tilkjent.stream()
            .filter(b -> b.getDagsats() > 0)
    ...
    public boolean erÅpenBehandlingMedSøknadRundtIM(Behandling behandling, LocalDate referanseDato) {
        ...
        var perioder = ytelsesFordelingRepository.hentAggregatHvisEksisterer(behandling.getId())
            ...
            .toList();
        var min = perioder.stream().map(OppgittPeriodeEntitet::getFom).min(Comparator.naturalOrder()).orElse(Tid.TIDENES_ENDE).minusWeeks(3);
        var max = perioder.stream().map(OppgittPeriodeEntitet::getTom).max(Comparator.naturalOrder()).orElse(Tid.TIDENES_BEGYNNELSE);
        return referanseDato.isAfter(min) && referanseDato.isBefore(max);
    }
    ...
    public boolean erFagsakPassendeForSøknadFamilieHendelse(VurderFagsystem vurderFagsystem, Fagsak fagsak, boolean vurderHenlagte) {
        ...
        // Finn behandling
        var behandling = vurderHenlagte ? behandlingRepository.hentSisteYtelsesBehandlingForFagsakId(
            fagsak.getId()) : behandlingRepository.finnSisteIkkeHenlagteYtelseBehandlingFor(fagsak.getId());
        if (behandling.isEmpty()) {
            return true;
        }

        return behandling.flatMap(this::hentAktuellFamilieHendelse).map(g -> erGrunnlagPassendeFor(g, fagsak.getYtelseType(), vurderFagsystem))
    ...
    public boolean erFagsakBasertPåInntektsmeldingUtenSøknad(Fagsak fagsak) {
        ...
        var harFamilieHendelse = behandlingRepository.hentSisteYtelsesBehandlingForFagsakId(fagsak.getId())
            ...
            .isPresent();
        var harMottattInntektsmelding = mottatteDokumentTjeneste.hentMottatteDokumentFagsak(fagsak.getId())
            .stream()
    ...
    public boolean kanFagsakBasertPåInntektsmeldingBrukesForSøknad(VurderFagsystem vurderFagsystem, Fagsak fagsak) {
        ...
        var referansedato = vurderFagsystem.getStartDatoForeldrepengerInntektsmelding()
            .filter(sd -> sd.isBefore(idagMinusSøknadsfrist))
            .orElse(idagMinusSøknadsfrist)
            .minus(MAKS_AVVIK_DAGER_IM_INPUT);
        return inntektsmeldingTjeneste.hentAlleInntektsmeldingerForFagsakInkludertInaktive(fagsak.getSaksnummer())
            .values()
            .stream()
    ...
    private boolean erGrunnlagPassendeFor(FamilieHendelseGrunnlagEntitet grunnlag, FagsakYtelseType ytelseType, VurderFagsystem vurderFagsystem) {
        ...
        if (FamilieHendelseType.gjelderFødsel(fhType)) {
            return familieHendelseTjeneste.matcherFødselsSøknadMedBehandling(grunnlag, vurderFagsystem.getBarnTermindato().orElse(null),
                vurderFagsystem.getBarnFodselsdato().orElse(null));
        }
        if (FamilieHendelseType.gjelderAdopsjon(fhType)) {
            return familieHendelseTjeneste.matcherOmsorgsSøknadMedBehandling(grunnlag, vurderFagsystem.getOmsorgsovertakelsedato().orElse(null),
    ...
    private boolean gjelderGrunnlagAnnenFamiliehendelse(FamilieHendelseGrunnlagEntitet grunnlag,
                                                        FagsakYtelseType ytelseType,
                                                        VurderFagsystem vurderFagsystem) {
        var fhType = grunnlag.getGjeldendeVersjon().getType();
        var bhTemaFagsak = BehandlingTema.fraFagsakHendelse(ytelseType, fhType);
        if (!vurderFagsystem.getBehandlingTema().erKompatibelMed(bhTemaFagsak)) {
            return false;
    ...
    public BehandlendeFagsystem vurderAktuelleFagsakerForInntektsmeldingFP(VurderFagsystem vurderFagsystem, List<Fagsak> saker) {
        ...
        var sorterteSaker = saker.stream().collect(Collectors.groupingBy(f -> sorterFagsakForStartdatoIM(f, startdatoIM)));

        if (!sorterteSaker.getOrDefault(SorteringSaker.GRUNNLAG_DATO_MATCH, List.of()).isEmpty()) {
            return behandlendeFagsystemFraFagsaker(sorterteSaker.get(SorteringSaker.GRUNNLAG_DATO_MATCH), SorteringSaker.GRUNNLAG_DATO_MATCH,
                vurderFagsystem);
        }
        if (!sorterteSaker.getOrDefault(SorteringSaker.GRUNNLAG_MULIG_MATCH, List.of()).isEmpty()) {
            return behandlendeFagsystemFraFagsaker(sorterteSaker.get(SorteringSaker.GRUNNLAG_MULIG_MATCH), SorteringSaker.GRUNNLAG_MULIG_MATCH,
                vurderFagsystem);
        }
        ...
        if (!tvilssaker.isEmpty()) {
            LOG.info("VurderFagsystem FP IM {} manuell pga nylige saker av type {} startdatoIM {} saker {}", vurderFagsystem.getJournalpostIdLog(),
                SorteringSaker.GRUNNLAG_MISMATCH, startdatoIM, tvilssaker);
            return new BehandlendeFagsystem(MANUELL_VURDERING);
        }
        if (!sorterteSaker.getOrDefault(SorteringSaker.INNTEKTSMELDING_DATO_MATCH, List.of()).isEmpty()) {
            return behandlendeFagsystemFraFagsaker(sorterteSaker.get(SorteringSaker.INNTEKTSMELDING_DATO_MATCH),
                SorteringSaker.INNTEKTSMELDING_DATO_MATCH, vurderFagsystem);
        }
        ...
        if (!tvilssakerIM.isEmpty()) {
            LOG.info("VurderFagsystem FP IM {} manuell pga nylige saker av type {} startdatoIM {} im-saker {}", vurderFagsystem.getJournalpostIdLog(),
                SorteringSaker.INNTEKTSMELDING_MISMATCH, startdatoIM, tvilssakerIM);
            return new BehandlendeFagsystem(MANUELL_VURDERING);
        }
        return sorterteSaker.getOrDefault(SorteringSaker.TOM_SAK, List.of()).isEmpty() ? new BehandlendeFagsystem(
            VEDTAKSLØSNING) : new BehandlendeFagsystem(VEDTAKSLØSNING, sorterteSaker.get(SorteringSaker.TOM_SAK).get(0).getSaksnummer());
    }
    ...
    private SorteringSaker sorterFagsakForStartdatoIM(Fagsak fagsak, LocalDate startdatoIM) {
        var sisteBehandling = behandlingRepository.hentÅpneYtelseBehandlingerForFagsakId(fagsak.getId())
            .stream()
            .findFirst()
        ...
        if (fhGrunnlag.isEmpty()) {
            ...
            var match = alleInntektsmeldinger.values()
                .stream()
                .flatMap(Collection::stream)
                .map(Inntektsmelding::getStartDatoPermisjon)
        ...
        try {
            ...
            if (førsteDagBehandling.minus(MAKS_AVVIK_DAGER_IM_INPUT).isBefore(startdatoIM) && førsteDagBehandling.plus(MAKS_AVVIK_DAGER_IM_INPUT)
                .isAfter(startdatoIM)) {
    ...
    private boolean kanFagsakUtenGrunnlagBrukesForDokument(VurderFagsystem vurderFagsystem, Behandling sisteBehandling) {
        ...
        var alleInntektsmeldinger = inntektsmeldingTjeneste.hentAlleInntektsmeldingerForFagsakInkludertInaktive(
            sisteBehandling.getFagsak().getSaksnummer());
        ...
        if (startdato.isPresent()) {
            ...
            return alleInntektsmeldinger.values()
                .stream()
                .flatMap(Collection::stream)
                .map(im -> im.getStartDatoPermisjon().orElseGet(() -> im.getInnsendingstidspunkt().toLocalDate()))
                .anyMatch(d -> oppgittFørsteDag.minus(MAKS_AVVIK_DAGER_IM_INPUT).isBefore(d) && oppgittFørsteDag.plus(MAKS_AVVIK_DAGER_IM_INPUT)
                    .isAfter(d));
        } else {
            return alleInntektsmeldinger.values()
                .stream()
                .flatMap(Collection::stream)
                .anyMatch(i -> i.getStartDatoPermisjon()
                    .orElseGet(() -> i.getInnsendingstidspunkt().toLocalDate())
                    .isAfter(innkommendeReferanseDato.minus(PERIODE_FOR_MULIGE_SAKER_IM)));
        }
    }
    ...
    public Optional<BehandlendeFagsystem> klageinstansUstrukturertDokumentVurdering(List<Fagsak> sakerTilVurdering) {
        var åpneFagsaker = sakerTilVurdering.stream().filter(this::harÅpenEllerNyligAvsluttetKlageEllerAnkeBehandlingKlageinstans).toList();
        return
            åpneFagsaker.size() != 1 ? Optional.empty() : Optional.of(new BehandlendeFagsystem(VEDTAKSLØSNING, åpneFagsaker.get(0).getSaksnummer()));
    }
    ...
    public Optional<BehandlendeFagsystem> vurderFagsystemKlageAnke(BehandlingTema behandlingTema, List<Fagsak> sakerTilVurdering) {
        ...
        var behandlinger = sakerTilVurdering.stream()
            .filter(f -> BehandlingTema.UDEFINERT.equals(behandlingTema) || BehandlingTema.gjelderSammeYtelse(behandlingTema,
                BehandlingTema.fraFagsak(f, null)))
        ...
        if (sakerMedKjenteNyereKlager.size() > 1) {
            ...
            return Optional.empty();
        } else if (sakerMedKjenteNyereKlager.size() == 1 && (behandlinger.isEmpty() || behandlinger.stream()
            .noneMatch(b -> sakerMedKjenteNyereKlager.contains(b.getFagsak().getSaksnummer())))) {
        ...
        if (behandlinger.isEmpty() && sakerTilVurdering.isEmpty()) {
            ...
            var sakerMedÅpenBehandling = sakerTilVurdering.stream()
                .filter(f -> !behandlingRepository.hentÅpneYtelseBehandlingerForFagsakId(f.getId()).isEmpty())
                .count();
            LOG.info("VFS-KLAGE ingen vedtak - antall saker {} saker med åpen behandling {} saksnummer {}", sakerTilVurdering.size(),
                sakerMedÅpenBehandling, saker);
        } else if (behandlinger.size() > 1) {
            var sistOpprettetSakMinusÅr = behandlinger.stream()
                .map(b -> b.getFagsak().getOpprettetTidspunkt())
                .max(Comparator.naturalOrder())
                .orElseGet(LocalDateTime::now)
                .minusMonths(12);
            var fagsakerSisteÅret = behandlinger.stream()
                .map(Behandling::getFagsak)
                .filter(f -> f.getOpprettetTidspunkt().isAfter(sistOpprettetSakMinusÅr))
                .toList();
            ...
            var saker = sakerTilVurdering.stream().map(Fagsak::getSaksnummer).toList();
            var sakerMedKlage = sakerTilVurdering.stream()
                .filter(f -> behandlingRepository.hentSisteBehandlingAvBehandlingTypeForFagsakId(f.getId(), BehandlingType.KLAGE).isPresent())
                .count();
            LOG.info("VFS-KLAGE flere saker - antall saker {} saker med klage {} saksnummer {}", sakerTilVurdering.size(), sakerMedKlage, saker);
        }
        return behandlinger.size() != 1 ? Optional.empty() : Optional.of(
            new BehandlendeFagsystem(VEDTAKSLØSNING, behandlinger.get(0).getFagsak().getSaksnummer()));
    }
