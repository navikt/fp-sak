...
public class InnhentDokumentTjeneste {
    ...
    public void utfør(MottattDokument mottattDokument, BehandlingÅrsakType behandlingÅrsakType) {
        ...
        var dokumentGruppe = brukDokumentKategori(dokumentTypeId, mottattDokument.getDokumentKategori()) ? DOKUMENTKATEGORI_TIL_GRUPPE.getOrDefault(
            mottattDokument.getDokumentKategori(), DokumentGruppe.VEDLEGG) : DOKUMENTTYPE_TIL_GRUPPE.getOrDefault(dokumentTypeId,
            DokumentGruppe.VEDLEGG);
    ...
    public void opprettFraTidligereBehandling(Long behandlingId,
                                              MottattDokument mottattDokument,
                                              BehandlingÅrsakType behandlingÅrsakType) { //#SXX og #IXX
        ...
        if (mottattDokument.erSøknadsDokument()) {
            ...
            throw new IllegalArgumentException(
                "Utvikler-feil: MottattDokument må være av type søknad eller inntektsmelding, var: " + mottattDokument.getDokumentType().getKode());
        }
        dokumentmottaker.opprettFraTidligereAvsluttetBehandling(fagsak, behandlingId, mottattDokument, behandlingÅrsakType,
            skalMottasSomKøet(fagsak));
    }
    ...
    private Dokumentmottaker finnMottaker(DokumentGruppe dokumentGruppe, FagsakYtelseType fagsakYtelseType) {
        ...
        if (selected.isAmbiguous()) {
            throw new IllegalArgumentException(
                "Mer enn en implementasjon funnet for DokumentGruppe=" + dokumentGruppe + ", FagsakYtelseType=" + fagsakYtelseType);
        }
        if (selected.isUnsatisfied()) {
            throw new IllegalArgumentException(
                "Ingen implementasjoner funnet for DokumentGruppe=" + dokumentGruppe + ", FagsakYtelseType=" + fagsakYtelseType);
        }
