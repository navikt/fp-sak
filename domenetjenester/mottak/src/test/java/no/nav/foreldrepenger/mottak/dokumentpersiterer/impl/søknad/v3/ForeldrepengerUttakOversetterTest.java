...
class ForeldrepengerUttakOversetterTest {
    ...
    void skalFiltrereBortIrrelevantUtsettelseSelvOmMorsAktivitetErSatt(MorsAktivitet morsAktivitet) {
    ...
    @Test
    void skalBeholdeSenereFriUtsettelseNårBareFarHarRett() {
        var uttak = lagUttaksperiode(IKRAFT_FRA_DATO);
        var fri = lagPeriode(IKRAFT_FRA_DATO.plusDays(2), UtsettelseÅrsak.FRI);

        assertThat(filtrer(Rettighetstype.BARE_FAR_RETT, uttak, fri)).containsExactly(uttak, fri);
    }

    @ParameterizedTest
    @EnumSource(value = Rettighetstype.class, names = {"BARE_FAR_RETT", "BARE_FAR_RETT_MOR_UFØR"})
    void skalBeholdeAllePerioderNårBareFarHarRett(Rettighetstype rettighetstype) {
        var uttak = lagUttaksperiode(IKRAFT_FRA_DATO);
        var arbeid = lagPeriode(IKRAFT_FRA_DATO.plusDays(2), UtsettelseÅrsak.ARBEID);
        var opphold = lagPeriode(IKRAFT_FRA_DATO.plusDays(4), OppholdÅrsak.FEDREKVOTE_ANNEN_FORELDER);

        assertThat(filtrer(rettighetstype, uttak, arbeid, opphold)).containsExactly(uttak, arbeid, opphold);
    }
    ...
    private static List<OppgittPeriodeEntitet> filtrer(OppgittPeriodeEntitet... perioder) {
        return filtrer(Rettighetstype.BEGGE_RETT, perioder);
    }

    private static List<OppgittPeriodeEntitet> filtrer(Rettighetstype rettighetstype, OppgittPeriodeEntitet... perioder) {
        return ForeldrepengerUttakOversetter.filtrerPerioderSomSkalSaksbehandles(List.of(perioder), rettighetstype);
    }
