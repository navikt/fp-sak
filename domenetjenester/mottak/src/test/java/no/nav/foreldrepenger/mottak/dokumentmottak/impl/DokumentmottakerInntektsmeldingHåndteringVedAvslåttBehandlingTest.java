...
public class DokumentmottakerInntektsmeldingHåndteringVedAvslåttBehandlingTest extends DokumentmottakerTestsupport {
    ...
    void setUp() {
        ...
        dokumentmottakerInntektsmelding = new DokumentmottakerInntektsmelding(dokumentmottakerFellesSpied, behandlingsoppretterSpied,
    ...
    void gittAvslåttBehandlingPgaManglendeDokMedIkkeUtløptFristForInnsendingSkalOppretteNyFørstegangsbehandling() {
        ...
        Mockito.doNothing()
            .when(dokumentmottakerFellesSpied)
            .opprettFørstegangsbehandlingMedHistorikkinslagOgKopiAvDokumenter(Mockito.any(), Mockito.any(), Mockito.any());
        Mockito.doReturn(true).when(dokumentmottakerFellesSpied).skalOppretteNyFørstegangsbehandling(any());

        var avslåttBehandling = opprettBehandling(FagsakYtelseType.FORELDREPENGER, BehandlingType.FØRSTEGANGSSØKNAD, BehandlingResultatType.AVSLÅTT,
        ...
        // Assert
        Mockito.verify(dokumentmottakerFellesSpied, Mockito.times(1))
            .opprettFørstegangsbehandlingMedHistorikkinslagOgKopiAvDokumenter(any(), any(), any());
    }
    ...
    void gittAvslåttBehandlingPgaManglendeDokMedUtløptFristForInnsendingSkalOppretteTaskForÅVurdereDokument() {
        ...
        var behandling = opprettBehandling(FagsakYtelseType.FORELDREPENGER, BehandlingType.FØRSTEGANGSSØKNAD, BehandlingResultatType.AVSLÅTT,
            Avslagsårsak.MANGLENDE_DOKUMENTASJON, VedtakResultatType.AVSLAG, DATO_ETTER_INNSENDINGSFRISTEN);
        ...
        // Assert
        Mockito.verify(behandlingsoppretterSpied, Mockito.never())
            .opprettNyFørstegangsbehandlingMedImOgVedleggFraForrige(any(), any(), any(), anyBoolean());
    ...
    void gittAvslåttBehandlingMenIkkePgaManglendeDokMedSkalOppretteTaskForÅVurdereDokument() {
        ...
        Mockito.doNothing().when(dokumentmottakerFellesSpied).opprettTaskForÅVurdereDokument(any(), any(), any());
        var behandling = opprettBehandling(FagsakYtelseType.FORELDREPENGER, BehandlingType.FØRSTEGANGSSØKNAD, BehandlingResultatType.AVSLÅTT,
            Avslagsårsak.FOR_LAVT_BEREGNINGSGRUNNLAG, VedtakResultatType.AVSLAG, DATO_ETTER_INNSENDINGSFRISTEN);
        var inntektsmelding = dummyInntektsmeldingDokument(behandling);
    ...
    void gittHenlagtBehandlingSkalOppretteVurderDokumentInntilVidere() {
        //Arrange
        Mockito.doNothing().when(dokumentmottakerFellesSpied).opprettTaskForÅVurdereDokument(any(), any(), any());
        var behandling = opprettBehandling(FagsakYtelseType.FORELDREPENGER, BehandlingType.FØRSTEGANGSSØKNAD,
            BehandlingResultatType.HENLAGT_SØKNAD_MANGLER, null, VedtakResultatType.UDEFINERT, DATO_ETTER_INNSENDINGSFRISTEN);
        var inntektsmelding = dummyInntektsmeldingDokument(behandling);
