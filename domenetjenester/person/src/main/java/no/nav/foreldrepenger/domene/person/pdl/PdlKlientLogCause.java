...
public class PdlKlientLogCause {
    ...
    public PersoninfoFalskIdentitet sjekkUtenIdentifikatorFalskIdentitet(AktørId aktørId) {
        return FalskIdentitet.finnFalskIdentitet(aktørId.getId(), pdlKlient)
            .map(pi -> new PersoninfoFalskIdentitet(pi.navn(), pi.fødselsdato(),
                LokalPersonMapper.mapKjønn(pi.kjønn()), mapStatsborgerskap(pi), PersonstatusType.UTPE))
            .orElse(null);
    }
