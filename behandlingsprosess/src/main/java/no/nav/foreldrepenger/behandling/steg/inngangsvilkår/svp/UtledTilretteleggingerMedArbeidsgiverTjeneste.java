...
class UtledTilretteleggingerMedArbeidsgiverTjeneste {
    ...
    @Inject
    UtledTilretteleggingerMedArbeidsgiverTjeneste(InntektArbeidYtelseTjeneste iayTjeneste, InntektsmeldingTjeneste inntektsmeldingTjeneste) {
    ...
    public List<SvpTilretteleggingEntitet> utled(Behandling behandling,
        ...
        var relevanteYrkesaktiviteter = new YrkesaktivitetFilter(arbeidsforholdInformasjonOpt, aktørArbeidFraRegisterOpt).getYrkesaktiviteter()
            .stream()
        ...
        List<SvpTilretteleggingEntitet> tilretteleggingerMedArbeidsforholdId = new ArrayList<>(
            tilretteleggingerMedArbeidsgiver.stream().filter(t -> t.getInternArbeidsforholdRef().isPresent()).toList());

        List<SvpTilretteleggingEntitet> tilretteleggingerUtenArbeidsforholdId = new ArrayList<>(
            tilretteleggingerMedArbeidsgiver.stream().filter(t -> t.getInternArbeidsforholdRef().isEmpty()).toList());
        ...
        if (!tilretteleggingerMedArbeidsforholdId.isEmpty()) {
            var arbeidsforholdIdgruppertPerArbeidsgiver = tilretteleggingerMedArbeidsforholdId.stream()
                .collect(Collectors.groupingBy(this::tilretteleggingNøkkel));
            List<SvpTilretteleggingEntitet> måVurderesPåNytt = finnTilretteleggingerSomMåVurderesPåNytt(inntektsmeldinger,
                arbeidsforholdIdgruppertPerArbeidsgiver);
            ...
            måVurderesPåNytt.forEach(tilr -> {
                if (tilretteleggingerUtenArbeidsforholdId.stream()
                    .map(SvpTilretteleggingEntitet::getArbeidsgiver)
                    .noneMatch(a -> a.equals(tilr.getArbeidsgiver()))) {
        ...
        for (var tilrettelegging : tilretteleggingerUtenArbeidsforholdId) {
            ...
            var arbeidsgiver = tilrettelegging.getArbeidsgiver()
            ...
            var inntektsmeldingerForArbeidsgiver = inntektsmeldinger.stream().filter(im -> arbeidsgiver.equals(im.getArbeidsgiver())).toList();
    ...
    List<SvpTilretteleggingEntitet> finnTilretteleggingerSomMåVurderesPåNytt(List<Inntektsmelding> kobledeInntektsmeldinger,
                                                                             Map<String, List<SvpTilretteleggingEntitet>> tilrMedArbeidsforholdsIdPerArbeidsgiverMap) {
        ...
        tilrMedArbeidsforholdsIdPerArbeidsgiverMap.forEach((key, value) -> {
            ...
            var alleIderHarMatchendeIm = value.stream()
                .map(SvpTilretteleggingEntitet::getInternArbeidsforholdRef)
                .allMatch(internArbeidsforholdRef -> finnesIdIListenAvInntektsmeldingerForArbeidsgiver(arbeidsgiver, kobledeInntektsmeldinger,
                    internArbeidsforholdRef.orElse(null)));
    ...
    private boolean finnesIdIListenAvInntektsmeldingerForArbeidsgiver(Arbeidsgiver arbeidsgvier,
    ...
    private List<SvpTilretteleggingEntitet> opprettTilretteleggingForHverYrkesaktivitet(List<Yrkesaktivitet> yrkesaktiviteter,
        ...
        return yrkesaktiviteter.stream()
            ...
            .map(yrkesaktivitet -> new SvpTilretteleggingEntitet.Builder(tilrettelegging).medInternArbeidsforholdRef(
                yrkesaktivitet.getArbeidsforholdRef()).build())
            .toList();
    }

    private boolean harAnsettelsesperiodeSomInkludererEllerTilkommerEtterStp(LocalDate stp, Yrkesaktivitet yrkesaktivitet) {
        return new YrkesaktivitetFilter(yrkesaktivitet).getAnsettelsesPerioder(yrkesaktivitet)
            .stream()
            .anyMatch(aktivitetsavtale -> aktivitetsavtale.getPeriode().inkluderer(stp) || aktivitetsavtale.getPeriode().getFomDato().isAfter(stp));
    }
