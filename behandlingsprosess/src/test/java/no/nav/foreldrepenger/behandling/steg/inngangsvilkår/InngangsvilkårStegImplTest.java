...
class InngangsvilkårStegImplTest {
    ...
    void skal_hoppe_til_uttak_ved_avslag_for_foreldrepenger_ved_revurdering() {
        ...
        assertThat(vilkårResultat.getVilkårene().stream().map(Vilkår::getGjeldendeVilkårUtfall).collect(toList())).containsExactly(
    ...
    void skal_gi_aksjonspunkt_ved_avslag_på_opptjening_for_foreldrepenger_ved_førstegang() {
        ...
        var inngangsvilkårFellesTjeneste = new InngangsvilkårFellesTjeneste(regelOrkestrerer, mock(SkjæringstidspunktTjeneste.class));
        // Act
        var stegResultat = new SutOpptjeningSteg(repositoryProvider, inngangsvilkårFellesTjeneste).utførSteg(kontekst);
    ...
    void skal_gi_aksjonspunkt_ved_avslag_på_opptjening_for_foreldrepenger_ved_revurdering() {
        ...
        Behandlingsresultat.builderEndreEksisterende(revurdering.getBehandlingsresultat()).medBehandlingResultatType(BehandlingResultatType.AVSLÅTT);
    ...
    void skal_ved_tilbakehopp_rydde_vilkårresultat_og_vilkår_og_behandlingsresultattype() {
        ...
        var inngangsvilkårFellesTjeneste = new InngangsvilkårFellesTjeneste(regelOrkestrerer, mock(SkjæringstidspunktTjeneste.class));
        // Act
        new SutMedlemskapsvilkårSteg(repositoryProvider, inngangsvilkårFellesTjeneste).vedTransisjon(kontekst, modell,
            BehandlingSteg.TransisjonType.HOPP_OVER_BAKOVER, null, null);

        // Assert
        var vilkårResultat = behandling.getBehandlingsresultat().getVilkårResultat();
        assertThat(vilkårResultat.getVilkårResultatType()).isEqualTo(VilkårResultatType.IKKE_FASTSATT);
        assertThat(vilkårResultat.getVilkårene().stream().map(Vilkår::getGjeldendeVilkårUtfall).collect(toList())).containsExactly(
            VilkårUtfallType.IKKE_VURDERT);
        assertThat(behandling.getBehandlingsresultat().getBehandlingResultatType()).isEqualTo(BehandlingResultatType.IKKE_FASTSATT);
    }
    ...
    public void skal_ved_fremoverhopp_rydde_avklarte_fakta_og_vilkår() {
        ...
        // Act
        new SutMedlemskapsvilkårSteg(repositoryProvider, inngangsvilkårFellesTjeneste).vedTransisjon(kontekst, modell,
            BehandlingSteg.TransisjonType.HOPP_OVER_FRAMOVER, null, null);
    ...
    class SutOpptjeningSteg extends VurderOpptjeningsvilkårSteg {
        ...
        SutOpptjeningSteg(BehandlingRepositoryProvider repositoryProvider, InngangsvilkårFellesTjeneste inngangsvilkårFellesTjeneste) {
