...
class ManuellRegistreringValidator {
    ...
    static void validerAktivitetskrav(ManuellRegistreringDto registreringDto, RelasjonsRolleType relasjonsRolleType) {
        ...
        } else if (registreringDto instanceof ManuellRegistreringForeldrepengerDto foreldrepengerDto && !ForeldreType.MOR.equals(
            registreringDto.getSoker()) && Optional.ofNullable(registreringDto.getAnnenForelder())
            .filter(AnnenForelderDto::getSokerHarAleneomsorg)
            .isEmpty()) {
