...
public class DokumentRestTjeneste {
    ...
    public Collection<MottattDokumentDto> hentAlleMottatteDokumenterForBehandling(@TilpassetAbacAttributt(supplierClass = BehandlingAbacSuppliers.UuidAbacDataSupplier.class) @NotNull @QueryParam(UuidDto.NAME) @Parameter(description = UuidDto.DESC) @Valid UuidDto uuidDto) {
        var behandling = behandlingRepository.hentBehandling(uuidDto.getBehandlingUuid());
        return mottatteDokumentRepository.hentMottatteDokumentMedFagsakId(behandling.getFagsakId()).stream().map(MottattDokumentDto::new).toList();
    }
    ...
    @BeskyttetRessurs(actionType = ActionType.READ, resourceType = ResourceType.FAGSAK)
    public Collection<DokumentDto> hentAlleDokumenterForSak(@TilpassetAbacAttributt(supplierClass = SaksnummerAbacSupplier.Supplier.class) @NotNull @QueryParam("saksnummer") @Parameter(description = "Saksnummer") @Valid SaksnummerDto saksnummerDto) {
        ...
        var åpneBehandlinger = behandlingRepository.hentÅpneYtelseBehandlingerForFagsakId(fagsakId)
            .stream()
            .map(Behandling::getId)
            .collect(Collectors.toSet());

        var inntektsMeldinger = inntektsmeldingTjeneste.hentAlleInntektsmeldingerForAngitteBehandlinger(åpneBehandlinger)
            .stream()
    ...
    @BeskyttetRessurs(actionType = ActionType.READ, resourceType = ResourceType.FAGSAK)
    public Response hentDokument(@TilpassetAbacAttributt(supplierClass = SaksnummerAbacSupplier.Supplier.class) @NotNull @QueryParam("saksnummer") @Parameter(description = "Saksnummer") @Valid SaksnummerDto saksnummer,
        ...
        try {
            ...
            var feilmelding = String.format("Dokument ikke funnet for journalpostId= %s dokumentId= %s", journalpostId.getJournalpostId(),
                dokumentId.getDokumentId());
    ...
    private List<DokumentDto> mapFraArkivJournalPost(ArkivJournalPost arkivJournalPost,
        ...
        if (arkivJournalPost.getAndreDokument() != null) {
            arkivJournalPost.getAndreDokument()
                .forEach(dok -> dokumentForJP.add(mapFraArkivDokument(arkivJournalPost, dok, mottatteIMDokument, inntektsMeldinger)));
        }
        return dokumentForJP;
    }

    private DokumentDto mapFraArkivDokument(ArkivJournalPost arkivJournalPost,
                                            ArkivDokument arkivDokument,
                                            Map<JournalpostId, List<MottattDokument>> mottatteIMDokument,
                                            Map<JournalpostId, List<Inntektsmelding>> inntektsMeldinger) {
        var dto = new DokumentDto(arkivJournalPost, arkivDokument);
        if (DokumentTypeId.INNTEKTSMELDING.equals(arkivDokument.getDokumentType()) && mottatteIMDokument.containsKey(
            arkivJournalPost.getJournalpostId())) {
            var behandlinger = mottatteIMDokument.get(dto.getJournalpostId())
                .stream()
            ...
            var behandlingUuidList = mottatteIMDokument.get(dto.getJournalpostId())
                .stream()
                .filter(imdok -> inntektsMeldinger.containsKey(dto.getJournalpostId()))
                .map(MottattDokument::getBehandlingId)
            ...
            var navn = inntektsMeldinger.getOrDefault(dto.getJournalpostId(), Collections.emptyList()).stream().map((Inntektsmelding inn) -> {
            ...
            var referanse = inntektsMeldinger.getOrDefault(dto.getJournalpostId(), Collections.emptyList())
                .stream()
