...
public class ForvaltningBehandlingRestTjeneste {
    ...
    @Operation(description = "Ta behandling av vent og henlegg",
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "Henlegger en åpen førstegangsbehandling og oppretter ny basert på siste søknad",
        tags = "FORVALTNING-behandling",
        responses = {
            @ApiResponse(responseCode = "200", description = "Ny behandling er opprettet.",
                content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    schema = @Schema(implementation = String.class)
                )
    ...
    public Response henleggÅpenFørstegangsbehandlingOgOpprettNy(@NotNull @QueryParam("saksnummer") @Valid SaksnummerDto saksnummerDto) {
        ...
        if (fagsak == null || FagsakStatus.LØPENDE.equals(fagsak.getStatus()) || FagsakStatus.AVSLUTTET.equals(fagsak.getStatus()) ||
            ...
            Optional<Behandling> behandling = behandlingRepository.hentSisteBehandlingAvBehandlingTypeForFagsakId(fagsak.getId(), BehandlingType.FØRSTEGANGSSØKNAD);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "Angi hvilken inntektsmelding som skal brukes ved samtidig mottatt. Send inn hvis åpen behandling.",
        tags = "FORVALTNING-behandling",
        responses = {
            @ApiResponse(responseCode = "200", description = "Inntektsmelding reinnsendt.",
                content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    schema = @Schema(implementation = String.class)
                )
    ...
    private void opprettMottaDokumentTask(Long fagsakId, MottattDokument mottattDokument) {
        ...
        prosessTaskData.setProperty(HåndterMottattDokumentTaskProperties.BEHANDLING_ÅRSAK_TYPE_KEY, BehandlingÅrsakType.RE_ENDRET_INNTEKTSMELDING.getKode());
        prosessTaskData.setCallIdFraEksisterende();
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "Starter ny revurdering (berørt)",
        tags = "FORVALTNING-behandling",
        responses = {
            @ApiResponse(responseCode = "200", description = "Ny behandling er opprettet.",
                content = @Content(
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "Oppdaterer migrert fra Infotrygd på gitt behandling",
        tags = "FORVALTNING-behandling",
        responses = {
            @ApiResponse(responseCode = "200", description = "Migrert fra Infotrygd oppdatert.",
                content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    schema = @Schema(implementation = String.class)
                )
    ...
    @BeskyttetRessurs(action = CREATE, ressurs = FAGSAK)
    @SuppressWarnings("findsecbugs:JAXRS_ENDPOINT")
    public Response oppdaterMigrertFraInfotrygd(@BeanParam @Valid ForvaltningBehandlingIdDto dto, @NotNull @QueryParam("migrertFraInfotrygd") @Valid Boolean migrertFraInfotrygd) {
