...
class FrontendLoginResourceTest {
    ...
    public void skal_hente_ut_relative_path_from_url() {
        Response response = resource.doLogin("http://google.com/asdf");
        assertThat(response.getLocation()).hasPath("/asdf");

        response = resource.doLogin("/");
        assertThat(response.getLocation()).hasPath("/");

        response = resource.doLogin("/fagsak/1234/behandling/1234/opptjening");
    ...
    public void innlogging_fra_fpsak_sak_web() {
        ...
        var response = resource.doLogin(basepath);
        assertThat(URI.create("/")).isEqualTo(response.getLocation());

        var hovedside = "/fpsak/";
        response = resource.doLogin(hovedside);
        ...
        response = resource.doLogin(medQueryOgFragment);
