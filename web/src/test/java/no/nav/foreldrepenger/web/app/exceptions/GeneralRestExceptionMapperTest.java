...
class GeneralRestExceptionMapperTest {
    ...
    void skalMappeValideringsfeil() {
        ...
        try (var response = exceptionMapper.toResponse(valideringsfeil)) {
            assertThat(response.getStatus()).isEqualTo(400);
            assertThat(response.getEntity()).isInstanceOf(FeilDto.class);
            var feilDto = (FeilDto) response.getEntity();

            assertThat(feilDto.feilkode()).isEqualTo(Feilkode.VALIDERING.name());
            assertThat(feilDto.feilmelding())
                .isEqualTo("Validering av felt [Et feltnavn]. Vennligst kontroller feltverdier: [Et feltnavn - En feilmelding]");
    ...
    void skalBehandlingEndretFeil() {
        try (var response = exceptionMapper.toResponse(new BehandlingEndretException())) {
            assertThat(response.getEntity()).isInstanceOf(FeilDto.class);
            var feilDto = (FeilDto) response.getEntity();
